...
class ExperimentConfig:
    ...
    horizons_ms: Dict[str, int] = field(default_factory=lambda: {
        "1h": 1 * MS_HOUR,
        "6h": 6 * MS_HOUR,
        "1d": 1 * MS_DAY,
        "3d": 3 * MS_DAY,
    })
    future_value_weights: Dict[str, float] = field(default_factory=lambda: {
        "long_view": 0.5,
        "like": 1.0,
        "comment": 1.0,
        "forward": 1.0,
    ...
    delay_conditions: List[str] = field(default_factory=lambda: [
    ...
    main_methods: List[str] = field(default_factory=lambda: [
    ...
    all_methods: List[str] = field(default_factory=lambda: [
        "arrival_time_naive",
...
def ensure_output_dirs(output_dir: Path) -> None:
    ...
    for subdir in [
        "raw", "processed", "summaries", "tables", "checks", "legacy", "logs", "metadata", "reports",
