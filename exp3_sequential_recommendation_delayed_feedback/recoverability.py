...
METHOD_META: dict[str, dict[str, Any]] = {
    "arrival_time_naive": {
        "method_display_name": "Arrival-time carrier", "information_interface": "arrival_time_carrier",
        "reference_role": "none", "diagnostic_only": False, "deployable": False,
    },
    "source_labelled_empirical": {
        "method_display_name": "Source-labelled empirical", "information_interface": "source_labelled",
        "reference_role": "source_labelled_control", "diagnostic_only": True, "deployable": False,
    },
    "source_aware_reference": {
        "method_display_name": "Source-aware reference", "information_interface": "offline_source_target",
        "reference_role": "source_aware_reference", "diagnostic_only": False, "deployable": False,
    },
    "partial_source_label_q10": {
        "method_display_name": "Partial source labels, q=0.10", "information_interface": "partial_source_label",
        "reference_role": "none", "diagnostic_only": False, "deployable": False,
    },
    "partial_source_label_q30": {
        "method_display_name": "Partial source labels, q=0.30", "information_interface": "partial_source_label",
        "reference_role": "none", "diagnostic_only": False, "deployable": False,
    },
    "partial_source_label_q50": {
        "method_display_name": "Partial source labels, q=0.50", "information_interface": "partial_source_label",
        "reference_role": "none", "diagnostic_only": False, "deployable": False,
    },
    "history_mean_static": {
        "method_display_name": "History mean static", "information_interface": "history_target_mean_only",
        "reference_role": "diagnostic_control", "diagnostic_only": True, "deployable": False,
    },
    "short_term_ridge_proxy": {
        "method_display_name": "Short-term ridge proxy", "information_interface": "lagged_short_term_proxy",
        "reference_role": "none", "diagnostic_only": False, "deployable": False,
    },
    "history_ewma_ridge_proxy": {
        "method_display_name": "History EWMA ridge proxy", "information_interface": "history_plus_lagged_proxy",
        "reference_role": "none", "diagnostic_only": True, "deployable": False,
    },
    "short_term_composite_surrogate": {
        "method_display_name": "Short-term composite surrogate", "information_interface": "lagged_composite_proxy",
        "reference_role": "none", "diagnostic_only": False, "deployable": False,
    },
}
...
class PreparedCondition:
    ...
    partial_update_cache: dict[tuple[float, int], tuple[np.ndarray, np.ndarray]] = field(default_factory=dict)
...
def _same_user_current_action_at_arrival_indices(events: pd.DataFrame, cfg: ExperimentConfig, action_to_idx: dict[str, int]) -> np.ndarray:
...
def _construct_timeline(main_events: pd.DataFrame, condition: str, actions: list[str], cfg: ExperimentConfig, history_action_signal: dict[str, float] | None = None) -> pd.DataFrame:
    target_col = f"y_long_value_log_{cfg.primary_horizon}"
    out = main_events.sort_values([cfg.time_col, cfg.user_col], kind="stable").reset_index(drop=True).copy()
    ...
    out["source_outcome"] = out[target_col].where(out[f"valid_for_{cfg.primary_horizon}"].eq(1), np.nan).astype(float)
    out["event_key"] = stable_uint64(
        out[cfg.user_col].astype(str) + "|" + out[cfg.video_col].astype(str) + "|" + out[cfg.time_col].astype(str) + "|" + out.index.astype(str)
    )
    ...
    fallback_signal = float(np.nanmean(list(history_action_signal.values()))) if history_action_signal else 0.0
    action_signal = out["action_bucket"].astype(str).map(history_action_signal or {}).fillna(fallback_signal).to_numpy(float)
    ...
    out["arrival_carrier_idx"] = _same_user_current_action_at_arrival_indices(out, cfg, action_to_idx)
    return out


def _evaluation_arrays(timeline: pd.DataFrame, actions: list[str], candidate_action_indices: np.ndarray, cfg: ExperimentConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
...
def _aggregated_updates(timeline: pd.DataFrame, n_actions: int) -> tuple[dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]], dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]], dict[int, ArrivalDelta]]:
    valid = timeline[timeline["source_outcome"].notna()].copy()
    source_updates: dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]] = {}
    carrier_updates: dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]] = {}
    event_arrivals: dict[int, ArrivalDelta] = {}
    ...
    valid["_bootstrap_user_idx"] = valid["_bootstrap_user_id"].map(user_to_idx).astype(int)
    ...
    for arrival_bin, group in valid.groupby("arrival_bin", sort=False):
        b = int(arrival_bin)
        source = group.groupby("action_idx")["source_outcome"].agg(["sum", "count"]).reset_index()
        source_updates[b] = (source["action_idx"].to_numpy(int), source["sum"].to_numpy(float), source["count"].to_numpy(float))
        carrier_group = group[group["arrival_carrier_idx"] >= 0].groupby("arrival_carrier_idx")["source_outcome"].agg(["sum", "count"]).reset_index()
        carrier_updates[b] = (carrier_group["arrival_carrier_idx"].to_numpy(int), carrier_group["sum"].to_numpy(float), carrier_group["count"].to_numpy(float))
...
def _make_user_arrays(timeline: pd.DataFrame, bins: np.ndarray, n_actions: int, cfg: ExperimentConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    valid = timeline[timeline["source_outcome"].notna()].copy()
    ...
    source_counts = np.zeros((len(users), len(arrival_bins), n_actions), dtype=np.float32)
    ...
    timeline.loc[valid.index, "_bootstrap_user_id"] = valid["_bootstrap_user_id"].to_numpy()
    timeline.loc[valid.index, "_bootstrap_user_idx"] = valid["_bootstrap_user_idx"].to_numpy()
    timeline.loc[valid.index, "_arrival_idx"] = valid["_arrival_idx"].to_numpy()
    return users, cell_sums, cell_counts, source_sums, source_counts, carrier_sums, carrier_counts
...
def _partial_deltas(prepared: PreparedCondition, q: float, seeds: list[int]) -> dict[int, dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]]]:
    ...
    for seed in seeds:
        ...
        for b, delta in prepared.event_arrivals.items():
            ...
            usable = idx >= 0
            sums = np.bincount(idx[usable], weights=delta.outcomes[usable], minlength=n_actions).astype(float)
...
def partial_user_update_arrays(prepared: PreparedCondition, q: float, seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for delta in prepared.event_arrivals.values():
        ...
        idx = np.where(labelled, delta.source_idx, delta.carrier_idx)
        ok = idx >= 0
        np.add.at(sums, (delta.user_idx[ok], delta.arrival_idx[ok], idx[ok]), delta.outcomes[ok].astype(np.float32))
        np.add.at(counts, (delta.user_idx[ok], delta.arrival_idx[ok], idx[ok]), 1.0)
    prepared.partial_update_cache[key] = (sums, counts)
    return sums, counts


def prepare_condition(main_events: pd.DataFrame, history_events: pd.DataFrame, actions: list[str], candidate_action_indices: np.ndarray, condition: str, cfg: ExperimentConfig = DEFAULT_CONFIG) -> PreparedCondition:
    history_target = f"y_long_value_log_{cfg.primary_horizon}"
    history_valid = history_events[history_events[f"valid_for_{cfg.primary_horizon}"].eq(1)].copy()
    history_action_signal = history_valid.groupby("action_bucket")[history_target].mean().to_dict()
    prior_target_mean = float(history_valid[history_target].mean())
    timeline = _construct_timeline(main_events, condition, actions, cfg, history_action_signal)
    bins, candidate_mask, reference_values, _ = _evaluation_arrays(timeline, actions, candidate_action_indices, cfg)
    if len(bins) == 0:
        raise ValueError("No eligible source bins after applying support restriction.")
    users, cell_sums, cell_counts, source_us, source_uc, carrier_us, carrier_uc = _make_user_arrays(timeline, bins, len(actions), cfg)
    source_updates, carrier_updates, event_arrivals = _aggregated_updates(timeline, len(actions))
    # Reattach because _make_user_arrays creates the canonical user/arrival indices.
    arrival_bins = np.sort(timeline.loc[timeline["source_outcome"].notna(), "arrival_bin"].unique())
    scores, _, coefficient_table = build_main_proxy_scores(main_events, history_events, actions, bins, cfg)
    calibration_rows: list[dict[str, Any]] = []
    for method in ["short_term_ridge_proxy", "history_ewma_ridge_proxy"]:
        for i, b in enumerate(bins):
            candidates = np.flatnonzero(candidate_mask[i])
            for a in candidates:
                calibration_rows.append({"method_id": method, "decision_bin": int(b), "bin_index": int(i), "action_idx": int(a), "action_bucket": actions[int(a)], "predicted_long_value_log": float(scores[method][int(b)][int(a)]), "observed_long_value_log": float(reference_values[i, a])})
    prep = PreparedCondition(
        condition=condition, actions=actions, candidate_action_indices=candidate_action_indices, bins=bins,
        candidate_mask=candidate_mask, reference_values=reference_values, prior_target_mean=prior_target_mean,
        source_updates=source_updates, carrier_updates=carrier_updates, event_arrivals=event_arrivals,
        arrival_bins=arrival_bins, proxy_scores=scores, proxy_prediction_rows=pd.DataFrame(calibration_rows),
        timeline=timeline, user_ids=users, user_cell_sums=cell_sums, user_cell_counts=cell_counts,
        user_source_update_sums=source_us, user_source_update_counts=source_uc,
        user_carrier_update_sums=carrier_us, user_carrier_update_counts=carrier_uc,
...
def _update_state_over_interval(state: ActionState, updates: dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]], last_decision: int | None, current_decision: int) -> None:
    for arrival_bin in sorted(updates):
        if arrival_bin < current_decision and (last_decision is None or arrival_bin >= last_decision):
            idx, sm, cnt = updates[arrival_bin]
            state.update(idx, sm, cnt)


def _run_dynamic_trajectory(prepared: PreparedCondition, method: str, cfg: ExperimentConfig, mask_updates: dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]] | None = None, replication_id: int = 0) -> list[dict[str, Any]]:
    ...
    for b in prepared.bins:
        ...
        rows.append({
            "delay_condition": prepared.condition, "method_id": method, "replication_id": replication_id,
            "replication_kind": "label_mask" if method.startswith("partial_") else "fixed_trajectory",
            "decision_bin": b, "method_action_idx": chosen, "method_action": prepared.actions[chosen],
            "oracle_action_idx": oracle_top, "oracle_action": prepared.actions[oracle_top],
            "method_value": float(values[chosen]), "oracle_value": float(values[oracle_top]),
            "ranking_regret": float(values[oracle_top] - values[chosen]), "top_action_match": int(chosen == oracle_top),
            "selected_top_k_action_indices": "|".join(map(str, top.tolist())), "oracle_top_k_action_indices": "|".join(map(str, oracle.tolist())),
...
def _run_proxy_trajectory(prepared: PreparedCondition, method: str, cfg: ExperimentConfig) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for r, b in enumerate(prepared.bins):
        ...
        chosen, oracle_top = int(top[0]), int(oracle[0])
        rows.append({
            "delay_condition": prepared.condition, "method_id": method, "replication_id": 0, "replication_kind": "fixed_history_fitted_proxy",
            "decision_bin": b, "method_action_idx": chosen, "method_action": prepared.actions[chosen],
            "oracle_action_idx": oracle_top, "oracle_action": prepared.actions[oracle_top],
            "method_value": float(values[chosen]), "oracle_value": float(values[oracle_top]),
            "ranking_regret": float(values[oracle_top] - values[chosen]), "top_action_match": int(chosen == oracle_top),
...
def _source_reference_rows(prepared: PreparedCondition, cfg: ExperimentConfig) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for r, b in enumerate(prepared.bins):
        candidates = np.flatnonzero(prepared.candidate_mask[r])
        values = prepared.reference_values[r]
        oracle = _top_indices(values, candidates, prepared.actions, cfg.main_top_k)
        rows.append({
            "delay_condition": prepared.condition, "method_id": "source_aware_reference", "replication_id": 0, "replication_kind": "offline_reference",
            "decision_bin": int(b), "method_action_idx": int(oracle[0]), "method_action": prepared.actions[int(oracle[0])],
            "oracle_action_idx": int(oracle[0]), "oracle_action": prepared.actions[int(oracle[0])],
            "method_value": float(values[int(oracle[0])]), "oracle_value": float(values[int(oracle[0])]),
            "ranking_regret": 0.0, "top_action_match": 1,
            "selected_top_k_action_indices": "|".join(map(str, oracle.tolist())), "oracle_top_k_action_indices": "|".join(map(str, oracle.tolist())),
            "n_candidate_actions": int(len(candidates)),
        })
    return rows
...
def _metadata_columns(raw: pd.DataFrame) -> pd.DataFrame:
    meta = pd.DataFrame.from_dict(METHOD_META, orient="index").reset_index(names="method_id")
    return raw.merge(meta, on="method_id", how="left")


def summary_from_raw(raw: pd.DataFrame, cfg: ExperimentConfig = DEFAULT_CONFIG) -> pd.DataFrame:
    per_rep = raw.groupby(["delay_condition", "method_id", "replication_id", "replication_kind"], dropna=False).agg(
        n_decision_bins=("decision_bin", "size"), ranking_regret_per_time_bin=("ranking_regret", "mean"),
        top_action_match_rate=("top_action_match", "mean"), policy_value=("method_value", "mean"), oracle_value=("oracle_value", "mean"),
    ).reset_index()
    out = per_rep.groupby(["delay_condition", "method_id", "replication_kind"], dropna=False).agg(
        point_estimate=("ranking_regret_per_time_bin", "mean"), top_action_match_rate=("top_action_match_rate", "mean"),
        policy_value=("policy_value", "mean"), oracle_value=("oracle_value", "mean"),
        n_label_mask_trajectories=("replication_id", "nunique"), n_decision_bins=("n_decision_bins", "max"),
    ).reset_index()
...
def arrival_summary(prepared: PreparedCondition) -> pd.DataFrame:
    ...
    for condition, g in t.groupby("delay_condition"):
        ...
        corr = np.corrcoef(g.loc[valid, "pseudo_delay_ms"].to_numpy(float), g.loc[valid, "action_value_signal"].to_numpy(float))[0, 1] if valid.sum() > 2 else np.nan
        rows.append({
            "delay_condition": condition, "n_arrival_messages": int(valid.sum()),
            "mean_delay_hours": float(np.mean(delays)), "sd_delay_hours": float(np.std(delays, ddof=1)),
            "p05_delay_hours": float(np.quantile(delays, .05)), "p50_delay_hours": float(np.quantile(delays, .5)), "p95_delay_hours": float(np.quantile(delays, .95)),
            "delay_history_action_signal_correlation": float(corr),
            "carrier_missing_rate": float((g.loc[valid, "arrival_carrier_idx"] < 0).mean()),
...
def oracle_action_dynamics_summary(raw: pd.DataFrame, cfg: ExperimentConfig = DEFAULT_CONFIG) -> pd.DataFrame:
...
def run_recoverability_experiment(main_events: pd.DataFrame, history_events: pd.DataFrame, actions: list[str], candidate_action_indices: np.ndarray, output_dir, cfg: ExperimentConfig = DEFAULT_CONFIG, replication_seeds: list[int] | None = None) -> dict[str, Any]:
    ...
    for condition in cfg.delay_conditions:
        prep = prepare_condition(main_events, history_events, actions, candidate_action_indices, condition, cfg)
        preps[condition] = prep
        proxy_coefficients.append(prep.proxy_coefficient_table.assign(delay_condition=condition))
        calibration_rows.append(prep.proxy_prediction_rows.assign(delay_condition=condition))
        ...
        for method in ["history_mean_static", "short_term_ridge_proxy", "history_ewma_ridge_proxy", "short_term_composite_surrogate"]:
        ...
        for q in cfg.partial_label_rates:
            ...
            for seed in seeds:
                all_raw.extend(_run_dynamic_trajectory(prep, method, cfg, updates[int(seed)], int(seed)))
        # Do not persist multi-GB timelines.  A per-condition compact audit is enough.
        audit = prep.timeline[["source_event_id", cfg.user_col, "source_time", "source_bin", "arrival_time", "arrival_bin", "action_bucket", "action_idx", "arrival_carrier_idx", "source_outcome", "pseudo_delay_ms", "delay_condition"]].sample(n=min(200_000, len(prep.timeline)), random_state=20260622).sort_values("source_time")
        save_dataframe(audit, output_dir / "processed" / f"arrival_timeline_audit_sample_{condition}.csv")
    raw = _metadata_columns(pd.DataFrame(all_raw)).sort_values(["delay_condition", "method_id", "replication_id", "decision_bin"]).reset_index(drop=True)
    ...
    save_dataframe(oracle_action_dynamics_summary(raw, cfg), output_dir / "summaries" / "oracle_action_dynamics_summary.csv")
    save_dataframe(pd.concat(delays, ignore_index=True), output_dir / "summaries" / "arrival_mechanism_summary.csv")
    save_dataframe(pd.concat(proxy_coefficients, ignore_index=True), output_dir / "tables" / "proxy_ridge_coefficients.csv")
    save_dataframe(pd.concat(calibration_rows, ignore_index=True), output_dir / "raw" / "proxy_calibration_cells_raw.csv")
    save_dataframe(pd.DataFrame.from_dict(METHOD_META, orient="index").reset_index(names="method_id"), output_dir / "metadata" / "method_information_contract.csv")
