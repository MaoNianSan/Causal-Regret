...
def required_input_paths(
    root: Path, cfg: ExperimentConfig = DEFAULT_CONFIG
) -> list[Path]:
    return [
...
def _load_log(
    root: Path, filename: str, cfg: ExperimentConfig, sample_users: int | None
) -> pd.DataFrame:
    df = read_csv_checked(resolve_input_path(root, filename))
    require_columns(
        df, [cfg.user_col, cfg.video_col, cfg.time_col, cfg.duration_col], filename
    )
...
def _normalize(df: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    keep = (
        (out[cfg.time_col] > 0)
        & (out[cfg.duration_col] > 0)
        & out[cfg.user_col].notna()
        & out[cfg.video_col].notna()
    )
...
def _attach_features(
    df: pd.DataFrame, tag_map: pd.DataFrame, cfg: ExperimentConfig, origin: str
) -> pd.DataFrame:
    out = df.merge(
        tag_map[[cfg.video_col, "primary_tag"]], on=cfg.video_col, how="left"
    )
    out["primary_tag"] = (
        out["primary_tag"].fillna(cfg.unknown_action_bucket).map(parse_primary_tag)
    )
    out["primary_tag"] = out["primary_tag"].replace(
        {"": cfg.unknown_action_bucket, "nan": cfg.unknown_action_bucket}
    )
    out["stream_origin"] = origin
    out["watch_ratio"] = (out[cfg.play_time_col] / (out[cfg.duration_col] + EPS)).clip(
        0.0, 1.0
    )
    ...
    return out.sort_values([cfg.user_col, cfg.time_col], kind="stable").reset_index(
        drop=True
    )


def _make_action_map(
    history_standard: pd.DataFrame, cfg: ExperimentConfig
) -> tuple[dict[str, str], pd.DataFrame, list[str], np.ndarray]:
    ...
    """
    valid = history_standard.loc[
        history_standard["primary_tag"].astype(str).ne(cfg.unknown_action_bucket)
    ].copy()
    ...
    if len(top_tags) < cfg.main_top_k:
        raise ValueError(
            "History split does not contain enough non-missing action tags for the requested top-k evaluation."
        )
    lookup = {tag: tag for tag in top_tags}
    actions = top_tags + [cfg.residual_action_bucket]
    table = (
        history_standard["primary_tag"]
        .astype(str)
        .value_counts(dropna=False)
        .rename_axis("primary_tag")
        .reset_index(name="n_history_standard_events")
    )
    table["action_bucket"] = table["primary_tag"].map(
        lambda x: lookup.get(x, cfg.residual_action_bucket)
    )
...
def _apply_action_vocab(
    df: pd.DataFrame, lookup: dict[str, str], cfg: ExperimentConfig
) -> pd.DataFrame:
    out = df.copy()
    out["action_bucket"] = (
        out["primary_tag"]
        .astype(str)
        .map(lambda x: lookup.get(x, cfg.residual_action_bucket))
    )
    return out
...
def write_precheck(root: Path, output_dir: Path, cfg: ExperimentConfig) -> None:
    ...
    for filename in [
        cfg.main_log,
        cfg.history_log,
        cfg.video_basic_file,
        cfg.random_log,
        cfg.user_feature_file,
        cfg.video_stat_file,
    ]:
        ...
        row = {
            ...
            "required_for_primary": filename
            in {cfg.main_log, cfg.history_log, cfg.video_basic_file},
            "exists": path.exists(),
            "n_columns": 0,
            "columns": "",
        }
        ...
        rows.append(row)
    save_dataframe(
        pd.DataFrame(rows), output_dir / "checks" / "input_schema_report.csv"
    )
...
def prepare_logs(
    ...
    history = _attach_features(
        _normalize(_load_log(root, cfg.history_log, cfg, sample_users), cfg),
        tag_map,
        cfg,
        "history_standard",
    )
    main = _attach_features(
        _normalize(_load_log(root, cfg.main_log, cfg, sample_users), cfg),
        tag_map,
        cfg,
        "main_standard",
    )

    vocab_lookup, action_map, actions, candidate_action_indices = _make_action_map(
        history, cfg
    )
    ...
    for name, frame in [("history_standard", history), ("main_standard", main)]:
        maybe_save_parquet(
            frame, output_dir / "processed" / f"{name}_processed.parquet"
        )
    save_dataframe(action_map, output_dir / "processed" / "fixed_action_bucket_map.csv")
    vocab = pd.DataFrame(
        {"action_index": np.arange(len(actions)), "action_bucket": actions}
    )
