...
from proxy_models import _feature_state_by_bin, action_proxy_cells, build_main_proxy_scores, history_mean_static_scores
...
def test_carrier_never_uses_future_exposure() -> None:
    ...
    carrier = _same_user_current_action_at_arrival_indices(events, cfg, {"a": 0, "b": 1, "c": 2})
...
def test_main_feature_state_ignores_future_bins() -> None:
    ...
    state_a = _feature_state_by_bin(base, bins, 1, cfg, initial_proxy_totals=(4.0, 20.0))
    state_b = _feature_state_by_bin(changed_future, bins, 1, cfg, initial_proxy_totals=(4.0, 20.0))
...
def test_static_control_table_reports_paired_effect() -> None:
    ...
    summary = pd.DataFrame([
        {"delay_condition": cfg.primary_delay_condition, "method_id": "history_mean_static", "point_estimate": .10, "ci_lower": .05, "ci_upper": .15, "ci_level": .95, "n_bootstrap": 10},
        {"delay_condition": cfg.primary_delay_condition, "method_id": "short_term_ridge_proxy", "point_estimate": .08, "ci_lower": .04, "ci_upper": .14, "ci_level": .95, "n_bootstrap": 10},
    ])
    effects = pd.DataFrame([{
        "delay_condition": cfg.primary_delay_condition, "method_id": "short_term_ridge_proxy",
        "comparator_method_id": "history_mean_static", "point_estimate": .02, "ci_lower": -.01, "ci_upper": .05,
    }])
