...
def _fixed_action_maps(
    raw: pd.DataFrame, prepared: PreparedCondition, cfg: ExperimentConfig
    ...
    subset = raw[
        (raw["delay_condition"] == prepared.condition)
        & raw["method_id"].isin(fixed_methods)
    ]
...
def _reference_actions(
    values: np.ndarray, mask: np.ndarray, actions: list[str], cfg: ExperimentConfig
...
def _replay_dynamic_actions(
    update_sums: np.ndarray,
    ...
    for bi, b in enumerate(prepared.bins):
        ...
        candidates = np.flatnonzero(
            prepared.candidate_mask[bi] & np.isfinite(values[bi])
        )
        if len(candidates):
...
def _metric_from_actions(
    values: np.ndarray,
    candidate_mask: np.ndarray,
    actions: list[str],
    chosen: np.ndarray,
    selected_topk: np.ndarray,
    cfg: ExperimentConfig,
) -> tuple[float, float, float]:
    oracle_top1, oracle_topk, oracle_value = _reference_actions(
        values, candidate_mask, actions, cfg
    )
    ...
    for b in np.flatnonzero(valid):
        ...
        denom = max(
            1,
            min(
                cfg.main_top_k, int(np.sum(candidate_mask[b] & np.isfinite(values[b])))
            ),
        )
        overlap[b] = len(p & o) / denom
    return (
        float(np.nanmean(regrets)),
        float(np.nanmean(match)),
...
def _calibration_rows(
    ...
    for method, frame in calibration.groupby("method_id", sort=False):
        ...
        try:
            work["decile"] = pd.qcut(
                work["predicted_long_value_log"].rank(method="first"),
                10,
                labels=False,
                duplicates="drop",
            )
            ...
            rows.append(
                {
                    ...
                    "method_id": method,
                    "decile": int(decile),
                    "mean_predicted_long_value_log": float(
                        group["predicted_long_value_log"].mean()
                    ),
                    "mean_observed_long_value_log": (
                        total_sum / total_count if total_count > 0 else np.nan
                    ),
                }
            )
    return rows
...
def run_user_bootstrap(
    ...
    if any(
        len(prep.user_ids) != common_n_users
        or not np.array_equal(prep.user_ids, first_prep.user_ids)
        for prep in prepared.values()
    ):
        raise ValueError(
            "Mechanism contrast requires identical ordered user sets across conditions."
        )
    shared_bootstrap_weights = [
        rng.multinomial(
            common_n_users, np.full(common_n_users, 1.0 / common_n_users)
        ).astype(np.float64)
    ...
    for condition, prep in prepared.items():
        ...
        partial_methods = {
            f"partial_source_label_q{int(round(q*100)):02d}": q
            for q in cfg.partial_label_rates
        }
        available_seeds = np.asarray(
            (
                cfg.fast_replication_seeds
                if len(
                    raw[raw["delay_condition"] == condition]["replication_id"].unique()
                )
                <= len(cfg.fast_replication_seeds) + 5
        ...
        route_seeds = raw[
            (raw["delay_condition"] == condition)
            & raw["method_id"].str.startswith("partial_source_label_", na=False)
        ]["replication_id"].unique()
        if len(route_seeds):
        ...
        for rep in range(n_bootstrap):
            ...
            values = np.divide(
                cell_sums,
                cell_counts,
                out=np.full_like(cell_sums, np.nan, dtype=float),
                where=cell_counts > 0,
            )
            oracle_top1, oracle_topk, _ = _reference_actions(
                values, prep.candidate_mask, prep.actions, cfg
            )
            ...
            all_rows.append(
                {
                    ...
                    "ranking_regret_per_time_bin": 0.0,
                    "top_action_match_rate": 1.0,
                    "top_k_overlap_with_source_aware_reference": 1.0,
                }
            )
            ...
            dynamic_specs = {
                "arrival_time_naive": (
                    prep.user_carrier_update_sums,
                    prep.user_carrier_update_counts,
                ),
                "source_labelled_empirical": (
                    prep.user_source_update_sums,
                    prep.user_source_update_counts,
                ),
            }
            for method, (u_sums, u_counts) in dynamic_specs.items():
                ...
                regret, match, overlap = _metric_from_actions(
                    values, prep.candidate_mask, prep.actions, chosen, topk, cfg
                )
                all_rows.append(
                    {
                        "bootstrap_rep": rep,
                        "delay_condition": condition,
                        "method_id": method,
                        "ranking_regret_per_time_bin": regret,
                        "top_action_match_rate": match,
                        "top_k_overlap_with_source_aware_reference": overlap,
                    }
                )
            ...
            for cal_row in _calibration_rows(
                cell_sums, cell_counts, prep.proxy_prediction_rows, cfg, rep
            ):
    ...
    raw_point = (
        raw.groupby(["delay_condition", "method_id"], sort=False)
        .agg(
            ranking_regret_per_time_bin=("ranking_regret", "mean"),
            top_action_match_rate=("top_action_match", "mean"),
        )
        .reset_index()
        .set_index(["delay_condition", "method_id"])
    )
    summary_rows = []
    for (condition, method), group in draws.groupby(
        ["delay_condition", "method_id"], sort=False
    ):
        lo, hi = percentile_ci(group["ranking_regret_per_time_bin"], cfg.ci_level)
        olo, ohi = percentile_ci(
            group["top_k_overlap_with_source_aware_reference"], cfg.ci_level
        )
        ...
        summary_rows.append(
            {
                ...
                **METHOD_META.get(method, {}),
                ...
                "point_estimate": float(raw_rec["ranking_regret_per_time_bin"]),
                "ci_lower": lo,
                "ci_upper": hi,
                "top_k_overlap_with_source_aware_reference": float(
                    group["top_k_overlap_with_source_aware_reference"].mean()
                ),
                "top_k_overlap_ci_lower": olo,
                "top_k_overlap_ci_upper": ohi,
                "top_action_match_rate": float(raw_rec["top_action_match_rate"]),
                "top_action_match_ci_lower": mlo,
                "top_action_match_ci_upper": mhi,
    ...
    for condition, group in draws.groupby("delay_condition", sort=False):
        base = group[group["method_id"] == "arrival_time_naive"][
            ["bootstrap_rep", "ranking_regret_per_time_bin"]
        ].rename(columns={"ranking_regret_per_time_bin": "base"})
        for method, target in group.groupby("method_id", sort=False):
            if method == "arrival_time_naive":
                continue
            merged = target[["bootstrap_rep", "ranking_regret_per_time_bin"]].merge(
                base, on="bootstrap_rep", how="inner"
            )
            reduction = merged["base"] - merged["ranking_regret_per_time_bin"]
            lo, hi = percentile_ci(reduction, cfg.ci_level)
            raw_base = float(
                raw_point.loc[
                    (condition, "arrival_time_naive"), "ranking_regret_per_time_bin"
                ]
            )
            raw_target = float(
                raw_point.loc[(condition, method), "ranking_regret_per_time_bin"]
            )
            effects_rows.append(
                {
                    "delay_condition": condition,
                    "method_id": method,
                    "metric_id": "ranking_regret_reduction_vs_arrival_time",
    ...
    for condition, group in draws.groupby("delay_condition", sort=False):
        base = group[group["method_id"] == static_control][
            ["bootstrap_rep", "ranking_regret_per_time_bin"]
        ].rename(columns={"ranking_regret_per_time_bin": "history_mean"})
        if base.empty:
            continue
        for method, target in group.groupby("method_id", sort=False):
            ...
            raw_base = float(
                raw_point.loc[
                    (condition, static_control), "ranking_regret_per_time_bin"
                ]
            )
            raw_target = float(
                raw_point.loc[(condition, method), "ranking_regret_per_time_bin"]
    ...
    for method in sorted(draws["method_id"].dropna().unique()):
        left = draws[
            (draws["delay_condition"] == coupled) & (draws["method_id"] == method)
        ][["bootstrap_rep", "ranking_regret_per_time_bin"]].rename(
            columns={"ranking_regret_per_time_bin": "coupled"}
        )
        right = draws[
            (draws["delay_condition"] == independent) & (draws["method_id"] == method)
        ][["bootstrap_rep", "ranking_regret_per_time_bin"]].rename(
            columns={"ranking_regret_per_time_bin": "independent"}
        )
        ...
        raw_left = (
            raw_point.loc[(coupled, method), "ranking_regret_per_time_bin"]
            if (coupled, method) in raw_point.index
            else np.nan
        )
        raw_right = (
            raw_point.loc[(independent, method), "ranking_regret_per_time_bin"]
            if (independent, method) in raw_point.index
            else np.nan
        )
    ...
    if not calibration.empty:
        calibration_summary = (
            ...
            .agg(
                ...
                ci_lower=(
                    "mean_observed_long_value_log",
                    lambda x: percentile_ci(x, cfg.ci_level)[0],
                ),
                ci_upper=(
                    "mean_observed_long_value_log",
                    lambda x: percentile_ci(x, cfg.ci_level)[1],
                ),
    ...
    save_dataframe(
        summary, output_dir / "summaries" / "user_bootstrap_metric_summary.csv"
    )
    save_dataframe(
        effects, output_dir / "summaries" / "paired_effect_vs_arrival_time.csv"
    )
    save_dataframe(
        static_effects,
    ...
    save_dataframe(
        mechanism_contrast, output_dir / "summaries" / "paired_mechanism_contrast.csv"
    )
    save_dataframe(
        calibration, output_dir / "raw" / "proxy_calibration_bootstrap_draws.csv"
    )
    save_dataframe(
        calibration_summary, output_dir / "summaries" / "proxy_calibration_summary.csv"
    )
