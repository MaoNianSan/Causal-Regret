...
def run_checks(run_tier: str) -> dict[str, Any]:
    ...
    learner_round = read_frame(
        output / "raw" / "exp1_learner_consequence_rounds.parquet"
    )
    route_seed = read_frame(output / "seed_metrics" / "exp1_route_seed_metrics.parquet")
    learner_seed = read_frame(
        output / "seed_metrics" / "exp1_learner_seed_metrics.parquet"
    )

    checks: list[dict[str, Any]] = []
    checks.append(
        _assert(not path_manifest.empty, "path_manifest_nonempty", len(path_manifest))
    )
    checks.append(
        _assert(not route_round.empty, "route_round_nonempty", len(route_round))
    )
    checks.append(
        _assert(not learner_round.empty, "learner_round_nonempty", len(learner_round))
    )

    expected_seeds = tuple(
        RUN.fast_seeds if run_tier == "fast" else RUN.evaluation_seeds
    )
    ...
    checks.append(
        _assert(
            ...
            "exact_seed_set",
            {"expected": expected_seeds, "observed": observed_seeds},
        )
    )
    observed_mechanisms = tuple(path_manifest.mechanism_id.drop_duplicates())
    checks.append(
        _assert(
            observed_mechanisms == tuple(MECHANISM_ORDER),
            "exact_mechanism_order",
            observed_mechanisms,
        )
    )
    ...
    checks.append(
        _assert(
            len(path_manifest) == expected_bundles,
            "complete_seed_mechanism_matrix",
            expected_bundles,
        )
    )
    ...
    checks.append(
        _assert(
            len(route_round) == expected_bundles * 2 * horizon,
            "exact_route_round_count",
            len(route_round),
        )
    )
    checks.append(
        _assert(
            len(learner_round) == expected_bundles * 2 * horizon,
            "exact_learner_round_count",
            len(learner_round),
        )
    )
    checks.append(
        _assert(
            set(route_round.route_id.unique()) == {"arrival_assigned", "source_bound"},
            "exact_route_set",
        )
    )
    checks.append(
        _assert(
            set(learner_round.feedback_binding_id.unique())
            == {"arrival_clock", "source_round"},
            "exact_binding_set",
        )
    )
    ...
    checks.append(
        _assert(
            ...
            [
                float(path_manifest.structural_loss_min.min()),
                float(path_manifest.structural_loss_max.max()),
            ],
        )
    )
    checks.append(
        _assert(
            int(path_manifest.loss_clipping_count.sum()) == 0,
            "no_loss_clipping",
            int(path_manifest.loss_clipping_count.sum()),
        )
    )
    checks.append(
        _assert(
            set(path_manifest.learner_prehistory_policy.unique())
            == {"cold_start_empty_queue"},
            "learner_prehistory_policy_frozen",
        )
    )
    ...
    for row in arrival_route.itertuples(index=False):
        ...
        update_mismatch += int(
            bool(row.route_map_updated) != bool(row.arrival_batch_size > 0)
        )
    checks.append(
        _assert(
            max(weight_errors or [0.0]) <= 1e-12,
            "uniform_arrival_batch_weights",
            max(weight_errors or [0.0]),
        )
    )
    checks.append(
        _assert(future_use == 0, "empty_clock_and_route_map_no_future_use", future_use)
    )
    checks.append(
        _assert(update_mismatch == 0, "route_map_updated_audit", update_mismatch)
    )
    ...
    checks.append(
        _assert(
            bool(
                np.allclose(
                    merged.selected_probability_arrival,
                    merged.selected_probability_source,
                )
            ),
    ...
    checks.append(
        _assert(
            bool(
                (merged.log_weight_hash_arrival == merged.log_weight_hash_source).all()
            ),
            "zero_delay_learner_states_identical",
            int(
                (merged.log_weight_hash_arrival != merged.log_weight_hash_source).sum()
            ),
        )
    )
    checks.append(
    ...
    checks.append(
        _assert(
            ...
            [
                float(learner_round.factual_loss.min()),
                float(learner_round.factual_loss.max()),
            ],
        )
    )

    source_updates = learner_round[learner_round.feedback_binding_id == "source_round"]
    arrival_updates = learner_round[
        learner_round.feedback_binding_id == "arrival_clock"
    ]
    source_mismatch = 0
    for row in source_updates.itertuples(index=False):
        source_mismatch += int(
            _as_list(row.updated_action_indices) != _as_list(row.arrived_source_actions)
        )
        source_mismatch += int(
            not np.allclose(
                [float(x) for x in _as_list(row.update_probabilities)],
                [float(x) for x in _as_list(row.arrived_source_probabilities)],
                atol=0.0,
                rtol=0.0,
            )
        )
    ...
    for row in arrival_updates.itertuples(index=False):
        arrival_mismatch += int(
            any(int(a) != int(row.action) for a in _as_list(row.updated_action_indices))
        )
        arrival_mismatch += int(
            any(
                int(c) != int(row.context_cell)
                for c in _as_list(row.updated_context_cells)
            )
        )
        arrival_mismatch += int(
            not np.allclose(
                [float(p) for p in _as_list(row.update_probabilities)],
    ...
    checks.append(
        _assert(
            source_mismatch == 0,
            "source_round_uses_source_action_and_probability",
            source_mismatch,
        )
    )
    checks.append(
        _assert(
            arrival_mismatch == 0,
            "arrival_clock_uses_preaction_current_binding",
            arrival_mismatch,
        )
    )
    tape_uniforms = (
        learner_round.groupby(["seed", "mechanism_id", "t"])["shared_action_uniform"]
        .nunique()
        .max()
    )
    checks.append(
        _assert(
            int(tape_uniforms) == 1,
            "shared_action_uniform_across_bindings",
            int(tape_uniforms),
        )
    )

    # Raw-to-seed reconstruction.
    reconstructed_route = route_round.groupby(
        ["seed", "mechanism_id", "route_id"], as_index=False
    ).agg(
    ...
    checks.append(
        _assert(
            ...
            "route_raw_to_seed_reconstruction",
            reconstruction_error,
        )
    )

    reconstructed_learner = learner_round.groupby(
        ["seed", "mechanism_id", "feedback_binding_id"], as_index=False
    ...
    learner_reconstruction_error = float(
        np.max(
            np.abs(
                compared_learner.structural_regret_raw
                - compared_learner.structural_regret_seed
            )
        )
    )
    checks.append(
    ...
    for mechanism in path_manifest.mechanism_id.unique():
        ...
        sample = np.sort(
            rng.choice(
                ...
                size=min(25, structural_loss.shape[0]),
                replace=False,
            )
        )
        fast_delta = action_gap_defect(
            route.route_loss_matrix[sample], structural_loss[sample]
        )
        brute_delta = action_gap_defect_bruteforce(
            route.route_loss_matrix[sample], structural_loss[sample]
        )
        delta_error = float(np.max(np.abs(fast_delta - brute_delta)))
        actions = rng.integers(
            0, structural_loss.shape[1], size=structural_loss.shape[0]
        )
        ...
        budget = float(
            np.sum(action_gap_defect(route.route_loss_matrix, structural_loss))
        )
        ...
        if (
            ...
            raise AssertionError(
                f"independent formula check failed: {formula_details[-1]}"
            )
    checks.append(
        _assert(True, "independent_delta_and_transfer_formula_checks", formula_details)
    )
        ...
        _assert(
            bool(
                (
                    ...
                    == np.isclose(
                        route_round.directed_choice_disagreement,
                        1.0,
                        atol=1e-12,
                        rtol=0.0,
                    )
    ...
    checks.append(
        _assert(
            ...
            and bool(route_seed.regret_stability_invariant_pass.all()),
    ...
    checks.append(
        _assert(
            ...
            {
                "exact_cardinal_shift": sweep_checks[
                    "theory_exact_cardinal_shift_sweep"
                ]["passed"],
    ...
    figure_data = pd.read_csv(
        output / "figures" / "data" / "fig_exp1_alignment_transfer_data.csv"
    )
    ...
    for mechanism in MECHANISM_ORDER:
        for panel, series, metric in (
            ...
            plotted = figure_data[
                (figure_data.mechanism_id == mechanism)
                & (figure_data.panel_id == panel)
                & (figure_data.series_id == series)
            ].iloc[0]
            source_value = route_summary[
                (route_summary.mechanism_id == mechanism)
                & (route_summary.route_id == "arrival_assigned")
                & (route_summary.metric_id == metric)
            ].iloc[0]
            reconstruction_errors.extend(
                abs(float(plotted[field]) - float(source_value[field]))
                for field in ("estimate", "ci_lower", "ci_upper")
            )
        for binding in ("arrival_clock", "source_round"):
            plotted = figure_data[
                (figure_data.mechanism_id == mechanism)
                & (figure_data.panel_id == "C")
                & (figure_data.series_id == binding)
            ].iloc[0]
            source_value = learner_summary[
        ...
        plotted = figure_data[
            (figure_data.mechanism_id == mechanism)
            & (figure_data.panel_id == "C")
            & (figure_data.series_id == "paired_contrast")
        ].iloc[0]
    ...
    checks.append(
        _assert(
            max(reconstruction_errors or [0.0]) <= 1e-12,
            "summary_to_figure_data_reconstruction",
            max(reconstruction_errors or [0.0]),
        )
    )
    ...
    for row in mechanism_table.itertuples(index=False):
        for metric, column in (
            ...
            source_value = route_summary[
                (route_summary.mechanism_id == row.mechanism_id)
                & (route_summary.route_id == "arrival_assigned")
                & (route_summary.metric_id == metric)
            ].iloc[0]
            table_errors.extend(
                abs(float(getattr(row, column + suffix)) - float(source_value[field]))
                for suffix, field in (
                    ("", "estimate"),
                    ("_ci_lower", "ci_lower"),
                    ("_ci_upper", "ci_upper"),
        ...
        table_errors.extend(
            abs(float(getattr(row, column)) - float(contrast[field]))
            for column, field in (
                ("arrival_minus_source_regret_rate", "estimate"),
                ("arrival_minus_source_ci_lower", "ci_lower"),
                ("arrival_minus_source_ci_upper", "ci_upper"),
            )
        )
    checks.append(
        _assert(
            max(table_errors or [0.0]) <= 1e-12,
            "summary_to_table_reconstruction",
            max(table_errors or [0.0]),
        )
    )

    figure_metadata_path = (
        output / "figures" / "data" / "fig_exp1_alignment_transfer_metadata.json"
    )
    figure_metadata = json.loads(figure_metadata_path.read_text(encoding="utf-8"))
    checks.append(
        _assert(
            figure_metadata.get("source_data_sha256")
            == sha256_file(
                output / "figures" / "data" / "fig_exp1_alignment_transfer_data.csv"
            ),
            "figure_data_sha256_matches_metadata",
        )
    )
    latex_text = (output / "tables" / "tab_exp1_mechanism_summary.tex").read_text(
        encoding="utf-8"
    )
    checks.append(
        _assert(
            "[" in latex_text and "]" in latex_text, "latex_table_contains_intervals"
        )
    )
    macro_text = (output / "manuscript" / "exp1_manuscript_macros.tex").read_text(
        encoding="utf-8"
    )
    values_payload = json.loads(
        (output / "manuscript" / "exp1_manuscript_values.json").read_text(
            encoding="utf-8"
        )
    )
    checks.append(
        _assert(
            "LearnerContrast" not in macro_text and "Alignment}{" not in macro_text,
            "nonpromoted_numerical_macros_withheld",
        )
    )
    checks.append(
        _assert(
            values_payload.get("manuscript_values_available") is False
            and values_payload.get("mechanisms") == {},
            "nonpromoted_manuscript_values_withheld",
        )
    )
    ...
    for frame_name, frame in (
        ("path", path_manifest),
        ("route", route_round),
        ("learner", learner_round),
    ):
        checks.append(
            _assert(
                frame.calibration_manifest_hash.nunique() == 1
                and frame.calibration_manifest_hash.iloc[0]
                == expected_calibration_hash,
                f"{frame_name}_calibration_lineage",
            )
        )
        checks.append(
            _assert(
                frame.generated_at.notna().all(), f"{frame_name}_generated_at_complete"
            )
        )

    refresh_output_manifest(output)
    manifest_payload = json.loads(
        (output / "metadata" / "artifact_manifest.json").read_text(encoding="utf-8")
    )
    absolute_paths = [
        record["path"]
        for record in manifest_payload.get("artifacts", [])
        if Path(record["path"]).is_absolute()
    ]
    checks.append(
        _assert(
            not absolute_paths,
            "artifact_manifest_uses_relative_paths",
            absolute_paths[:3],
        )
    )
    ...
    actual_hashes = {
        key: hash_payload(calibration[key])
        for key in ("structural", "delay", "misbinding", "context")
    }
    checks.append(
        _assert(
            manifest_hashes == actual_hashes,
            "calibration_artifact_hashes_match",
            actual_hashes,
        )
    )
    memo_texts = [
    ...
    unapproved = [
        text for text in memo_texts if "approved_status: approved" not in text
    ]
    ...
    report = {
        ...
        "paper_promotion_status": (
            "ELIGIBLE_FOR_PROMOTION_REVIEW"
            if run_tier == "full"
            else "FAST_NOT_PAPER_ELIGIBLE"
        ),
        "paper_result": False,
        "checks": checks,
...
def main() -> None:
    ...
    print(
        f"report={PROJECT_ROOT / 'outputs' / run_tier / 'checks' / 'exp1_validation_report.json'}"
    )
