...
def validate_frozen_configuration(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if bool(config["cohort"].get("prohibit_modal_campaign_fallback", False)) is not True:
    ...
    if float(config["routes"]["time_decay_rate_per_day"]) != 0.5:
        raise ConfigurationError("Time-decay rate is frozen at 0.5 per day.")
    checks.append({"check": "time_decay_rate", "status": "PASS"})
    return checks
...
def validate_run(
    ...
    *,
    ...
    expected_repetitions = int(
        ...
        else config["statistics"]["fast_repetitions" if mode == "fast" else "full_repetitions"]
    )
    ...
    kendall_outside_count = int(
        arrival_displacement["kendall_tau_b_point_outside_percentile_ci"]
        .fillna(False)
        .astype(bool)
        .sum()
        + source_route_pairwise["kendall_tau_b_point_outside_percentile_ci"]
        .fillna(False)
        .astype(bool)
        .sum()
    )
    ...
    for frame_name, frame in (
        ...
        values = pd.to_numeric(frame["bootstrap_repetitions"], errors="coerce").dropna().unique()
        if len(values) != 1:
            raise ScientificInvariantError(
    ...
    if bootstrap_audit is not None:
        reported["bootstrap_audit"] = int(bootstrap_audit["bootstrap_repetitions"])
    ...
    checks.append(
        {
            "check": "reported_bootstrap_repetitions_consistent",
            "status": "PASS",
            "value": reported,
        }
    )

    outside_column = "allocation_tv_point_outside_percentile_ci"
    outside_count = int(
