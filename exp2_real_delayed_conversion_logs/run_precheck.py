...
def main() -> None:
    parser = argparse.ArgumentParser(description="Precheck Criteo delayed-conversion dataset for Experiment 2.")
    parser.add_argument("--config", default="config_exp2.yaml")
    parser.add_argument("--fast-nrows", type=int, default=None, help="Optional row cap for debugging.")
    ...
    campaign_stats = defaultdict(lambda: {"n_impressions": 0, "n_conversion_rows": 0, "n_attributed_rows": 0, "n_clicked_rows": 0})
    conversion_stats = defaultdict(_new_conversion_stat)
    delays = []

    for chunk_id, chunk in enumerate(read_chunks(cfg, usecols=None, nrows=args.fast_nrows)):
    ...
    time_span_days = (max_ts - min_ts) / SECONDS_PER_DAY if np.isfinite(min_ts) and np.isfinite(max_ts) else np.nan
    configured_window_days = float(cfg["data"].get("observation_window_days", time_span_days if np.isfinite(time_span_days) else 0))
    analysis_start_ts = max(min_ts, max_ts - configured_window_days * SECONDS_PER_DAY) if np.isfinite(max_ts) else np.nan
    analysis_window = {
    ...
    (out_dir(cfg, "precheck") / "exp2_analysis_window.json").write_text(json.dumps(analysis_window, indent=2), encoding="utf-8")
    dataset_summary = pd.DataFrame([
        {"metric": "n_rows", "value": n_rows},
        {"metric": "n_users_exact", "value": len(uids) if uids is not None else np.nan},
        ...
        {"metric": "row_conversion_rate", "value": n_conversion_rows / n_rows if n_rows else np.nan},
        {"metric": "row_attribution_rate", "value": n_attributed_rows / n_rows if n_rows else np.nan},
        {"metric": "row_click_rate", "value": n_clicked_rows / n_rows if n_rows else np.nan},
        {"metric": "min_timestamp", "value": min_ts},
        {"metric": "max_timestamp", "value": max_ts},
        {"metric": "time_span_days", "value": time_span_days},
        {"metric": "configured_observation_window_days", "value": configured_window_days},
    ...
    campaign_summary = pd.DataFrame([{"campaign": k, **v} for k, v in campaign_stats.items()])
    min_conv = int(cfg["action"].get("min_conversion_rows_per_campaign", 0))
    eligible_campaign_summary = campaign_summary[campaign_summary["n_conversion_rows"] >= min_conv].copy()
    minimum_actions = int(cfg["action"].get("candidate_campaign_count_minimum", 1))
    if len(eligible_campaign_summary) < minimum_actions:
        raise RuntimeError(f"Only {len(eligible_campaign_summary)} campaigns satisfy min_conversion_rows_per_campaign={min_conv}; need at least {minimum_actions}.")
    campaign_summary = eligible_campaign_summary
    campaign_summary["conversion_rate"] = campaign_summary["n_conversion_rows"] / campaign_summary["n_impressions"].replace(0, np.nan)
    campaign_summary["click_rate"] = campaign_summary["n_clicked_rows"] / campaign_summary["n_impressions"].replace(0, np.nan)
    campaign_summary["impression_coverage_pct"] = 100 * campaign_summary["n_impressions"] / max(n_rows, 1)
    campaign_summary["conversion_coverage_pct"] = 100 * campaign_summary["n_conversion_rows"] / max(n_conversion_rows, 1)
    campaign_summary["attribution_coverage_pct"] = 100 * campaign_summary["n_attributed_rows"] / max(n_attributed_rows, 1)
    ...
    for k in cfg["action"].get("sensitivity_top_k", [10, 20, 50]):
        ...
        topk_rows.append({
            "n_campaigns": int(k),
            "impression_coverage_pct": float(100 * top["n_impressions"].sum() / max(n_rows, 1)),
            "conversion_coverage_pct": float(100 * top["n_conversion_rows"].sum() / max(n_conversion_rows, 1)),
            "attribution_coverage_pct": float(100 * top["n_attributed_rows"].sum() / max(n_attributed_rows, 1)),
            "min_conversion_rows": int(top["n_conversion_rows"].min()) if len(top) else 0,
            "min_attributed_rows": int(top["n_attributed_rows"].min()) if len(top) else 0,
        })
    write_csv(pd.DataFrame(topk_rows), out_dir(cfg, "precheck") / "exp2_top_campaign_coverage_summary.csv")
    ...
    delay_summary = pd.DataFrame([{ 
        ...
        "p25_seconds": float(np.quantile(delays, 0.25)) if len(delays) else np.nan,
        "p75_seconds": float(np.quantile(delays, 0.75)) if len(delays) else np.nan,
        "p90_seconds": float(np.quantile(delays, 0.90)) if len(delays) else np.nan,
        "p95_seconds": float(np.quantile(delays, 0.95)) if len(delays) else np.nan,
        "p99_seconds": float(np.quantile(delays, 0.99)) if len(delays) else np.nan,
        "max_seconds": float(np.max(delays)) if len(delays) else np.nan,
    }])
    for c in list(delay_summary.columns):
        if c.endswith("seconds"):
            delay_summary[c.replace("seconds", "days")] = delay_summary[c] / SECONDS_PER_DAY
    write_csv(delay_summary, out_dir(cfg, "precheck") / "exp2_delay_summary.csv")

    if len(delays):
        labels = ["<=1h", "1-6h", "6-24h", "1-7d", "7-30d"]
        bins = [-np.inf, 3600, 6 * 3600, SECONDS_PER_DAY, 7 * SECONDS_PER_DAY, 30 * SECONDS_PER_DAY + 1]
        ...
        delay_bucket_summary = delay_bucket.groupby("delay_bucket", observed=False).agg(
            n_conversion_rows=("delay_seconds", "size"),
            mean_delay_seconds=("delay_seconds", "mean"),
            median_delay_seconds=("delay_seconds", "median"),
        ).reset_index()
        delay_bucket_summary["share_pct"] = 100 * delay_bucket_summary["n_conversion_rows"] / len(delays)
        delay_bucket_summary["mean_delay_days"] = delay_bucket_summary["mean_delay_seconds"] / SECONDS_PER_DAY
        delay_bucket_summary["median_delay_days"] = delay_bucket_summary["median_delay_seconds"] / SECONDS_PER_DAY
    else:
        delay_bucket_summary = pd.DataFrame()
    write_csv(delay_bucket_summary, out_dir(cfg, "precheck") / "exp2_delay_bucket_summary.csv")
    ...
    if not conversion_summary.empty:
        conversion_summary["source_window_length_seconds"] = conversion_summary["max_timestamp"] - conversion_summary["min_timestamp"]
        conversion_summary["source_window_length_days"] = conversion_summary["source_window_length_seconds"] / SECONDS_PER_DAY
    write_csv(conversion_summary, out_dir(cfg, "precheck") / "exp2_conversion_id_summary.csv")

    if not conversion_summary.empty:
        n_cids = len(conversion_summary)
        check = pd.DataFrame([
            {"metric": "n_conversion_ids", "value": n_cids},
            {"metric": "share_unique_attributed_row", "value": float((conversion_summary["n_attributed_rows"] == 1).mean())},
            {"metric": "share_no_attributed_row", "value": float((conversion_summary["n_attributed_rows"] == 0).mean())},
            {"metric": "share_multiple_attributed_rows", "value": float((conversion_summary["n_attributed_rows"] > 1).mean())},
            {"metric": "mean_candidate_rows_per_conversion", "value": float(conversion_summary["n_candidate_rows"].mean())},
            {"metric": "median_candidate_rows_per_conversion", "value": float(conversion_summary["n_candidate_rows"].median())},
            {"metric": "p90_candidate_rows", "value": float(conversion_summary["n_candidate_rows"].quantile(0.90))},
            {"metric": "p99_candidate_rows", "value": float(conversion_summary["n_candidate_rows"].quantile(0.99))},
        ])
    else:
        check = pd.DataFrame()
    ...
    save_run_metadata(cfg, status="precheck_success", extra={"n_rows_processed": n_rows, "analysis_window": analysis_window, "eligible_action_count": len(campaign_summary)})
