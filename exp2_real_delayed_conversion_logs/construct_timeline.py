...
def _uid_integrity_filter(candidates: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if frame.empty:
        raise RuntimeError("No candidate rows with a valid conversion_id remain after identifier normalisation.")
    ...
    valid_ids = set(audit.loc[audit["uid_integrity_status"].eq("valid_one_uid"), "conversion_id"].astype(str))
    ...
    summary = pd.DataFrame(
        [
            ...
            {"metric": "candidate_rows_missing_conversion_id", "value": invalid_conversion_rows},
            {"metric": "candidate_rows_after_conversion_id_filter", "value": int(len(frame))},
...
def _build_action_cells(impressions: pd.DataFrame, candidates: pd.DataFrame, cfg: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    imp["source_day_index"] = pd.to_numeric(imp["source_day_index"], errors="coerce").astype("Int64")
    ...
    imp["cost"] = pd.to_numeric(imp.get("cost", 0.0), errors="coerce").fillna(0.0).clip(lower=0.0)
    cell_summary = imp.groupby(["campaign", "source_day_index"], as_index=False).agg(
        n_impressions=("campaign", "size"),
        total_cost=("cost", "sum"),
        mean_cost=("cost", "mean"),
    )
    candidate_counts = candidates.groupby(["candidate_campaign", "source_day_index"], as_index=False)["conversion_id"].nunique().rename(columns={"candidate_campaign": "campaign", "conversion_id": "n_candidate_conversions"})
    cell_summary = cell_summary.merge(candidate_counts, on=["campaign", "source_day_index"], how="left")
    cell_summary["n_candidate_conversions"] = cell_summary["n_candidate_conversions"].fillna(0).astype(int)
    cell_summary["eligible_decision_cell"] = cell_summary["n_impressions"].ge(min_impressions).astype(int)
    ...
    eligible = eligible.sort_values(["source_day_index", "campaign"], kind="stable").reset_index(drop=True)
    eligible["action_id"] = np.arange(len(eligible), dtype=np.int64)
    eligible["decision_cell_key"] = eligible["campaign"].astype(str) + "__d" + eligible["source_day_index"].astype(str)
    mapping = eligible[["action_id", "decision_cell_key", "campaign", "source_day_index", "n_impressions", "total_cost", "mean_cost", "n_candidate_conversions"]].copy()

    imp = imp.merge(mapping[["campaign", "source_day_index", "action_id", "decision_cell_key"]], on=["campaign", "source_day_index"], how="inner", validate="many_to_one")
    cand = candidates.copy()
    cand["candidate_campaign"] = normalise_identifier(cand["candidate_campaign"])
    cand["source_day_index"] = pd.to_numeric(cand["source_day_index"], errors="coerce").astype("Int64")
...
def _arrival_anchor_by_day(impressions: pd.DataFrame) -> pd.DataFrame:
    ...
    counts = impressions.groupby(["source_day_index", "campaign", "action_id"], as_index=False).size().rename(columns={"size": "n_impressions"})
    counts = counts.sort_values(["source_day_index", "n_impressions", "campaign", "action_id"], ascending=[True, False, True, True], kind="stable")
    active = counts.drop_duplicates("source_day_index", keep="first").copy()
    active = active.rename(columns={"source_day_index": "arrival_day_index", "campaign": "arrival_anchor_campaign", "action_id": "arrival_time_action_id"})
    return active[["arrival_day_index", "arrival_anchor_campaign", "arrival_time_action_id", "n_impressions"]]


def _conversion_table(candidates: pd.DataFrame, active: pd.DataFrame, attribution_col: str, click_col: str) -> pd.DataFrame:
    rows: list[dict] = []
    active_map = active.set_index("arrival_day_index")["arrival_time_action_id"].to_dict()
    for conversion_id, group in candidates.groupby("conversion_id", sort=False):
        ...
        if len(uid_values) != 1:
            raise RuntimeError(f"UID integrity contract violated after filtering for conversion_id={conversion_id!r}.")
        conversion_timestamp = float(pd.to_numeric(group["conversion_timestamp"], errors="coerce").iloc[0])
        ...
        rows.append(
            {
                ...
                "n_clicked_candidate_rows": int(_safe_numeric(group, click_col).eq(1).sum()),
                "n_attributed_candidate_rows": int(len(attributed)),
                "unique_labelled_conversion_candidates_flag": int(labelled is not None),
                "labelled_source_action_id": int(labelled["action_id"]) if labelled is not None else np.nan,
                "labelled_source_campaign": str(labelled["candidate_campaign"]) if labelled is not None else pd.NA,
                "labelled_source_day_index": int(labelled["source_day_index"]) if labelled is not None else np.nan,
                "first_candidate_timestamp": float(pd.to_numeric(group["candidate_timestamp"], errors="coerce").min()),
                "last_candidate_timestamp": float(pd.to_numeric(group["candidate_timestamp"], errors="coerce").max()),
                "delay_from_first_candidate_days": float((conversion_timestamp - pd.to_numeric(group["candidate_timestamp"], errors="coerce").min()) / SECONDS_PER_DAY),
                "delay_from_last_candidate_days": float((conversion_timestamp - pd.to_numeric(group["candidate_timestamp"], errors="coerce").max()) / SECONDS_PER_DAY),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Construct source-time decision-cell timelines for Experiment 2.")
    ...
    start, end = float(analysis_window["analysis_window_start_timestamp"]), float(analysis_window["analysis_window_end_timestamp"])

    ts, uid, campaign = get_col(cfg, "timestamp"), get_col(cfg, "uid"), get_col(cfg, "campaign")
    conversion, conversion_ts, conversion_id = get_col(cfg, "conversion"), get_col(cfg, "conversion_timestamp"), get_col(cfg, "conversion_id")
    attribution, click, cost = get_col(cfg, "attribution"), get_col(cfg, "click"), get_col(cfg, "cost")
    ...
    campaigns = campaign_frame.sort_values(["n_impressions", "campaign"], ascending=[False, True], kind="stable").head(n_campaigns)["campaign"].dropna().astype(str).tolist()
    ...
    write_csv(pd.DataFrame({"campaign": campaigns, "campaign_rank": range(1, len(campaigns) + 1)}), processed / "exp2_campaign_mapping.csv")

    raw_impressions = processed / "exp2_impression_timeline_raw.csv"
    raw_candidates = processed / "exp2_candidate_sources_raw.csv"
    for path in (raw_impressions, raw_candidates, processed / "exp2_impression_timeline.csv", processed / "exp2_candidate_sources.csv"):
        if path.exists():
            path.unlink()
    row_offset = 0
    rows_seen = 0
    ...
    for chunk_index, chunk in enumerate(read_chunks(cfg, usecols=None, nrows=args.fast_nrows), start=1):
        chunk = add_time_columns(chunk, cfg)
        chunk = chunk[_safe_numeric(chunk, ts).between(start, end, inclusive="both")].copy()
        if chunk.empty:
            continue
        ...
        conversion_with_valid_timing = _safe_numeric(chunk, conversion).eq(1) & conv_times.gt(timestamps) & np.isfinite(timestamps) & np.isfinite(conv_times)
        ...
        print(f"[construct] processed chunk {chunk_index}, retained rows={rows_seen:,}", flush=True)

    if not raw_impressions.exists() or not raw_candidates.exists():
        raise RuntimeError("No mapped impressions or conversion candidates were written.")
    impressions_raw = pd.read_csv(raw_impressions, dtype={"uid": "string", "conversion_id": "string", "campaign": "string"})
    candidates_raw = pd.read_csv(raw_candidates, dtype={"uid": "string", "conversion_id": "string", "candidate_campaign": "string"})
    ...
    uid_summary = pd.concat(
        [
            ...
            pd.DataFrame(
                [
                    {"metric": "candidate_rows_missing_conversion_id_before_persistence", "value": candidate_rows_missing_conversion_id},
                    {"metric": "candidate_rows_conversion_after_analysis_end_excluded", "value": candidate_rows_after_analysis_end},
                ]
            ),
        ],
        ignore_index=True,
    ...
    impressions, candidates, action_map, action_cell_audit = _build_action_cells(impressions_raw, candidates_uid, cfg)
    ...
    action_exposure = action_map[["action_id", "decision_cell_key", "campaign", "source_day_index", "n_impressions", "total_cost", "mean_cost"]].copy()
    ...
    conversions["main_cohort_eligible"] = (
        ...
        & pd.to_numeric(conversions["arrival_time_action_id"], errors="coerce").between(0, n_cells - 1)
    ...
    conversions["source_linked_audit_eligible"] = (
        ...
        & pd.to_numeric(conversions["labelled_source_action_id"], errors="coerce").between(0, n_cells - 1)
    ).astype(int)
    write_csv(conversions, processed / "exp2_conversion_arrivals.csv")
    write_csv(conversions[conversions["main_cohort_eligible"].eq(1)].copy(), processed / "exp2_all_conversion_candidates.csv")
    write_csv(conversions[conversions["source_linked_audit_eligible"].eq(1)].copy(), processed / "exp2_unique_labelled_conversion_candidates.csv")
