...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a non-paper synthetic Exp2 fixture."
    )
                ...
                rows.append(
                    {
                        ...
                        "conversion_id": -1,
                        "attribution": 0,
                        "click": int((day + campaign + rep) % 7 == 0),
                ...
                [
                    {
                        ...
                        "conversion_id": cid,
                        "attribution": int(first_label),
                        "click": int((day + rep) % 3 == 0),
                        "cost": round(0.42 + 0.01 * left, 4),
                    },
                    {
                        "timestamp": t1,
                        "uid": uid,
                        ...
                        "conversion_timestamp": conversion_ts,
                        "conversion_id": cid,
                        "attribution": int(not first_label),
                        "click": int((day + rep) % 4 != 0),
                        "cost": round(0.42 + 0.01 * right, 4),
                    },
                ]
            )
            conversion_index += 1
    ...
    rows += [
        {
            ...
            "campaign": "0",
            "conversion": 1,
            "conversion_timestamp": d + 8_000,
            "conversion_id": "cv_missing_uid",
            "attribution": 1,
            "click": 1,
            "cost": 0.4,
        },
        ...
        },
        {
            ...
            "conversion": 1,
            "conversion_timestamp": d + 8_000,
            "conversion_id": "cv_uid_minus_one",
            "attribution": 1,
            "click": 1,
            "cost": 0.4,
        },
    ...
    pd.DataFrame(rows).sort_values(["timestamp", "uid"], kind="stable").to_csv(
        output, sep="\t", index=False
    )
    print(
        f"synthetic fixture: {output} ({len(rows):,} rows; {conversion_index} valid conversion IDs)"
    )
