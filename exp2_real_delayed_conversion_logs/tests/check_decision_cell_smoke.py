...
def main() -> None:
    parser = argparse.ArgumentParser(description="Smoke-check a completed synthetic Exp2 run.")
    ...
    summary = pd.read_csv(processed / "exp2_conversion_uid_integrity_summary.csv").set_index("metric")["value"]
    ...
    core_pairs = divergence[divergence["route_left"].isin(core) & divergence["route_right"].isin(core)]
    ...
    assert abs(float(arrival["credit_allocation_tv_distance_vs_arrival_anchor"])) < 1e-12
    assert abs(float(arrival["top_k_decision_cell_overlap_vs_arrival_anchor"]) - 1.0) < 1e-12
    ...
    assert {"pairwise_credit_allocation_tv_distance", "pairwise_top_k_overlap"}.issubset(figure_pairwise.columns)
    diag = figure_pairwise[figure_pairwise["route_left"].eq(figure_pairwise["route_right"])]
    ...
    assert set(panel_b["method_id"]) == {"arrival_bin_anchor", "first_click", "last_click", "linear_attribution", "time_decay_soft"}
    assert set(panel_b["x_id"]) == {"credit_allocation_tv_distance_vs_arrival_anchor"}
    assert not bool(panel_b["paper_result"].any())

    metadata = pd.read_json(output_root / "figures" / "metadata" / "fig_exp2_attribution_sensitivity_metadata.json", typ="series")
    assert metadata["metric_formula_id"]
    assert metadata["paper_result"] is False

    formal = (ROOT / "docs" / "latex_interface_experiments.md").read_text(encoding="utf-8")
    for old_id in [
