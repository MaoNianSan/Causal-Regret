...
def build_primary_cohort(
    ...
    manifest["exclusion_reason"] = np.select(conditions, reasons, default="retained")
    manifest["is_primary_eligible"] = manifest["exclusion_reason"].eq("retained")
    ...
    for reason, count in manifest["exclusion_reason"].value_counts().items():
        if reason != "retained":
    ...
    audit = {
        ...
        "exclusion_counts": {
            str(key): int(value)
            for key, value in manifest["exclusion_reason"].value_counts().items()
        },
    }
