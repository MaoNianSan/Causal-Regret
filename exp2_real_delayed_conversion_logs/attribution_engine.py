...
ROUTE_META = {
    "arrival_bin_anchor": (
        ...
        True,
        False,
        "constructed arrival-bin modal campaign-source-day anchor",
    ),
    "first_click": (
        "source_candidate",
        "none",
        False,
        True,
        "first clicked source cell; first source event if no click",
    ),
    "last_click": (
        "source_candidate",
        "none",
        False,
        True,
        "last clicked source cell; last source event if no click",
    ),
    "linear_attribution": (
        "source_candidate",
        "none",
        False,
        True,
        "equal credit across unique candidate source-time decision cells",
    ),
    "time_decay_soft": (
        "source_candidate",
        "none",
        False,
        True,
        "credit proportional to exponential recency weight",
    ),
    "soft_attribution_em": (
        "source_candidate",
        "diagnostic_control",
        True,
        False,
        "EM-style exposure-calibrated responsibility allocator",
    ),
    "last_touch": (
        "source_candidate",
        "none",
        False,
        True,
        "most recent candidate source event",
    ),
    "uniform_soft": (
        "source_candidate",
        "none",
        False,
        True,
        "equal credit across candidate source-event rows",
    ),
    "click_prior_soft": (
        "source_candidate",
        "none",
        False,
        True,
        "recency weight multiplied by click prior",
    ),
    "source_linked_reference": (
        ...
        False,
        False,
        "single Criteo-attributed candidate source cell; audit only",
    ),
}
...
def _prepare_candidates(
    candidates: pd.DataFrame, conversion_ids: Iterable[str], window_days: float
) -> pd.DataFrame:
    ids = set(
        normalise_conversion_identifier(pd.Series(list(conversion_ids), dtype="string"))
        .dropna()
        .astype(str)
    )
    ...
    if frame["uid"].isna().any():
        raise RuntimeError(
            "UID integrity contract violated: candidate rows with missing UID reached route assignment."
        )
    for column in (
        "action_id",
        "candidate_timestamp",
    ...
    frame = frame[
        frame["action_id"].notna()
        & frame["candidate_timestamp"].notna()
        & frame["conversion_timestamp"].notna()
    ].copy()
    frame["action_id"] = frame["action_id"].astype(int)
    frame["delay_to_conversion_days"] = (
        (frame["conversion_timestamp"] - frame["candidate_timestamp"]) / 86400.0
    ).clip(lower=0.0)
    frame = frame[frame["delay_to_conversion_days"].le(float(window_days))].copy()
    if frame.empty:
        raise RuntimeError(
            f"No mapped candidate rows remain within candidate window={window_days} days."
        )
    ...
    if not invalid.empty:
        ...
        raise RuntimeError(
            f"UID integrity contract violated: {len(invalid)} conversion IDs have non-unique UID after candidate preparation; examples={examples}"
        )
    frame["conversion_id"] = frame["conversion_id"].astype(str)
    frame["uid"] = frame["uid"].astype(str)
    return frame.sort_values(
        ["conversion_id", "candidate_timestamp", "row_id"], kind="stable"
    ).reset_index(drop=True)


def _hard_rows(
    conversions: pd.DataFrame, route: str, action_column: str, cohort_id: str
) -> pd.DataFrame:
    frame = conversions[
        ["conversion_id", "uid", "conversion_timestamp", action_column]
    ].copy()
...
def _row_choice_rows(
    candidates: pd.DataFrame, route: str, cohort_id: str, chooser
) -> pd.DataFrame:
    rows: list[pd.Series] = []
    ...
    chosen = chosen[
        ["conversion_id", "uid", "conversion_timestamp", "action_id"]
    ].rename(columns={"action_id": "candidate_action_id"})
...
def _aggregate_weight_rows(
    candidates: pd.DataFrame, route: str, raw_weight: np.ndarray, cohort_id: str
) -> pd.DataFrame:
    frame = candidates[
        ["conversion_id", "uid", "conversion_timestamp", "action_id"]
    ].copy()
    frame["raw_weight"] = np.asarray(raw_weight, dtype=float)
    frame["raw_weight"] = np.where(
        np.isfinite(frame["raw_weight"]) & (frame["raw_weight"] > 0),
        frame["raw_weight"],
        0.0,
    )
    denominator = frame.groupby("conversion_id", sort=False)["raw_weight"].transform(
        "sum"
    )
    frame["weight"] = np.divide(
        frame["raw_weight"],
        denominator,
...
def _linear_by_unique_action(candidates: pd.DataFrame, cohort_id: str) -> pd.DataFrame:
    dedup = (
        ...
        .drop_duplicates(["conversion_id", "action_id"], keep="first")
        .copy()
    )
...
def _em_weights(
    candidates: pd.DataFrame, action_exposure: pd.DataFrame, cfg: dict
    ...
    exposures = np.asarray(
        [max(float(exposure_map.get(int(action), 0.0)), 1.0) for action in actions],
        dtype=float,
    )
    action_idx = candidates["action_id"].map(action_index).to_numpy(dtype=int)
    click = (
        pd.to_numeric(candidates.get("click", 0.0), errors="coerce")
        .fillna(0.0)
    ...
    conversion_codes, _ = pd.factorize(
        candidates["conversion_id"].astype(str), sort=False
    )
    ...
    for iteration in range(int(settings["max_iter"])):
        ...
        sum_by_conversion = np.bincount(
            conversion_codes, weights=raw, minlength=n_conversions
        )
    ...
    diagnostic = pd.DataFrame(
        {
            "action_id": actions,
            "em_log_effect": log_effect,
            "em_exposure": exposures,
            "em_credited_conversion_mass": np.bincount(
                action_idx, weights=weights, minlength=len(actions)
            ),
            "em_iterations_max": int(settings["max_iter"]),
        }
    )
...
def build_assignments(
    ...
    conv = conv[
        conv["conversion_id"].notna()
        & conv["conversion_id"].astype(str).isin(valid_ids)
    ].copy()
    if conv["uid"].isna().any():
        raise RuntimeError(
            "UID integrity contract violated: main conversion table contains missing UID after filtering."
        )
    ...
    for route in routes:
        if route == "arrival_bin_anchor":
            ...
            frames.append(
                _hard_rows(conv, route, "labelled_source_action_id", cohort_id)
            )
        elif route == "first_click":

            def first_choice(group: pd.DataFrame) -> pd.Series:
                clicked = group[
                    pd.to_numeric(group.get("click", 0.0), errors="coerce")
                    .fillna(0)
                    .eq(1)
                ]
                return (
                    (clicked if not clicked.empty else group)
                    .sort_values(["candidate_timestamp", "row_id"], kind="stable")
                    .iloc[0]
                )
            ...
            frames.append(
                _row_choice_rows(
                    ...
                    lambda group: group.sort_values(
                        ["candidate_timestamp", "row_id"], kind="stable"
                    ).iloc[-1],
                )
            )
            ...
            frames.append(
                _aggregate_weight_rows(
                    c, route, np.ones(len(c), dtype=float), cohort_id
                )
            )
            ...
            frames.append(
                _aggregate_weight_rows(
                    c,
                    route,
                    np.exp(-lam * c["delay_to_conversion_days"].to_numpy(dtype=float)),
                    cohort_id,
                )
            )
            ...
            raw = np.exp(-lam * c["delay_to_conversion_days"].to_numpy(dtype=float)) * (
                1.0
                + alpha
                * pd.to_numeric(c.get("click", 0.0), errors="coerce")
                .fillna(0.0)
                .to_numpy(dtype=float)
            )
    ...
    if result["conversion_id"].isna().any() or result["uid"].isna().any():
        raise RuntimeError(
            "UID/conversion identifier integrity contract violated in route assignments."
        )
    ...
    meta = pd.DataFrame(
        [
            {"route": route, **_route_metadata(route)}
            for route in sorted(result["route"].unique())
        ]
    )
    ...
    return RouteBuildResult(
        result.sort_values(
            ["cohort_id", "route", "conversion_id", "candidate_action_id"],
            kind="stable",
        ).reset_index(drop=True),
        em_diagnostic,
    )
