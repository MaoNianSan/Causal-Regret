...
def run_targeted_analyses(
    *,
    ...
    window_values = [int(config["cohort"]["candidate_window_days"])] + [
        int(value) for value in config["cohort"].get("targeted_candidate_window_days", [])
    ]
    window_cohorts: dict[int, CohortBuildResult] = {
        int(config["cohort"]["candidate_window_days"]): primary_cohort
    }
    ...
    for window in window_values[1:]:
        ...
        window_config["cohort"]["candidate_window_days"] = window
        try:
