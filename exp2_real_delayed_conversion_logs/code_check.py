...
def static_checks(rows: list[dict]) -> None:
    ...
    _add(rows, "all_python_files_compile", not failures, "; ".join(failures) or f"{len(python_files)} files")
    ...
    _add(rows, "all_conversion_main_cohort", "main_cohort_id: all_conversion_candidates" in config and "action_unit: campaign_source_day_cell" in config, "all conversion candidates use source-time decision cells")
    _add(rows, "source_linked_audit_only", "source_linked_reference" in text and "source_reference_audit_only" in check, "source-linked reference is restricted by semantic check")
    _add(rows, "uid_bootstrap", "multinomial_uid_cluster_bootstrap" in config and "rng.integers" in stats and "uid_bootstrap" in stats, "UID-cluster percentile bootstrap implemented")
    _add(rows, "main_figure_contract", "fig_exp2_attribution_sensitivity" in plot and "credit_allocation_tv_distance_vs_arrival_anchor" in plot and "top_k_decision_cell_overlap_vs_arrival_anchor" in plot and "Share of eligible source events (%)" in plot, "main figure displays allocation TV point-range and top-10 overlap annotations")
    _add(rows, "appendix_pairwise_overlap_figure", "fig_app_exp2_source_route_pairwise_overlap" in plot and "pairwise_credit_allocation_tv_distance" in plot and "pairwise_top_k_overlap" in plot, "appendix uses pairwise TV and top-10 overlap heatmaps")
    _add(rows, "appendix_diagnostics_as_tables", all(token in text for token in ["tbl_app_exp2_source_linked_audit", "tbl_app_exp2_candidate_window_sensitivity", "tbl_app_exp2_em_assignment_diagnostic"]), "source-linked, window, and EM diagnostics are tables")
    _add(rows, "no_replay_as_main_endpoint", "sequential_replay.py" not in runner and "mean_daily_logged_reference_regret" not in stats, "deprecated temporal replay removed from formal pipeline")
    _add(rows, "no_label_availability_leakage", "label_availability" not in stats and "label_availability" not in plot, "Exp2 does not reuse invalid label-mask sensitivity")
    _add(rows, "honest_estimand_language", "source-time" in config.lower() and "not online policy evaluation" in config.lower() and "not causal regret" in config.lower(), "configuration preserves observational estimand boundary")
    _add(rows, "source_time_cell_design", "campaign_source_day_cell" in config and "core_source_routes" in config and "at_least_one_core_pair_tv_above_threshold" in check, "source-time cells plus core-route scientific degeneracy gate")
    ...
    _add(rows, "deprecated_ids_removed_from_formal_code", not found, "no old Exp2 route or figure ids in formal code: " + str(found))
    name_violations = re.findall(r"figure_id\s*=\s*[\"']([^\"']+)[\"']", text)
    _add(rows, "figure_ids_snake_case", all(re.fullmatch(r"[a-z0-9_]+", item) for item in name_violations), "formal figure ids are snake_case: " + str(name_violations))
    latex_interface = (ROOT / "docs" / "latex_interface_experiments.md").read_text(encoding="utf-8")
    ...
    _add(
        ...
        "formal LaTeX figure interface contains required figure ids: " + str(figure_ids),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Static and completed-run code check for Experiment 2.")
    ...
    if not args.static_only:
        ...
        if not config.exists():
            _add(rows, "completed_run_exists", False, f"missing effective config: {config}")
        else:
            ...
            try:
                ...
                status = {}
            _add(rows, "completed_run_status", _completed_through_tables(status, args.mode), json.dumps(status, ensure_ascii=False))
            result = subprocess.run([sys.executable, "self_check.py", "--mode", args.mode, "--config", str(config)], cwd=ROOT)
            _add(rows, "self_check_passes", result.returncode == 0, f"exit_code={result.returncode}")
    output = ROOT / "outputs" / (args.mode if not args.static_only else "code_check") / "checks"
    ...
    (output / "code_check_report.md").write_text(
        ...
        + "\n".join(f"- [{row['status']}] {row['check_name']}: {row['detail']}" for row in rows),
        encoding="utf-8",
    )
