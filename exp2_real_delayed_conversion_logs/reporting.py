...
PAIR_LABELS = {
    ("first_touch", "last_touch"): "First–Last",
    ("first_touch", "linear_credit"): "First–Linear",
    ("first_touch", "time_decay_credit"): "First–Decay",
    ("last_touch", "linear_credit"): "Last–Linear",
    ("last_touch", "time_decay_credit"): "Last–Decay",
    ("linear_credit", "time_decay_credit"): "Linear–Decay",
}
...
def make_main_figure(
    ...
    *,
    ...
    figure_data_arrival = arrival[
        [
            "route_id",
            "allocation_tv_vs_arrival",
            "allocation_tv_ci_lower",
            "allocation_tv_ci_upper",
            "top_k",
            "top_k_overlap_vs_arrival",
            "top_k_overlap_ci_lower",
            "top_k_overlap_ci_upper",
        ]
    ].copy()
    figure_data_arrival["panel"] = "a"
    figure_data_arrival["display_label"] = figure_data_arrival["route_id"].map(
        route_display_label
    )

    figure_data_pairwise = pairwise[
        [
            "route_left",
            "route_right",
            "pair_label",
    ...
    figure_data_pairwise["panel"] = "b"

    figure_data_path = output_dir / "figure_exp2_main_data.csv"
    combined = pd.concat(
        [
    ...
    fig, (ax_a, ax_b) = plt.subplots(
        ...
        2,
        figsize=(
            float(plots["main_figure_width_in"]),
            float(plots["main_figure_height_in"]),
        ),
        gridspec_kw={"width_ratios": [1.02, 1.38], "wspace": 0.50},
    )

    # Panel (a): displacement from the arrival-time diagnostic anchor.
    y = np.arange(len(arrival))[::-1]
    point = arrival["allocation_tv_vs_arrival"].to_numpy(dtype=float)
    lower = arrival["allocation_tv_ci_lower"].to_numpy(dtype=float)
    upper = arrival["allocation_tv_ci_upper"].to_numpy(dtype=float)
    ax_a.hlines(y, lower, upper, linewidth=float(plots["line_width"]))
    ax_a.scatter(point, y, s=float(plots["marker_size"]) ** 2, zorder=3)
    cap = 0.08
    ax_a.vlines(lower, y - cap, y + cap, linewidth=0.8)
    ax_a.vlines(upper, y - cap, y + cap, linewidth=0.8)
    ax_a.set_yticks(y)
    ax_a.set_yticklabels([route_display_label(route) for route in route_order])
    ax_a.set_xlim(0.0, 1.0)
    ax_a.set_xlabel("Allocation TV distance from arrival-time anchor")
    ax_a.grid(axis="x", alpha=0.25, linewidth=0.6)
    for y_value, row in zip(y, arrival.itertuples(index=False), strict=True):
        ax_a.text(
            0.98,
            y_value,
            _shared_annotation(
                row.top_k_overlap_vs_arrival,
                row.top_k_overlap_ci_lower,
                row.top_k_overlap_ci_upper,
                int(row.top_k),
            ),
    ...
    # Panel (b): discrete route-pair comparisons, not a continuous scatter relation.
    y_pair = np.arange(len(pairwise))[::-1]
    pair_point = pairwise["allocation_tv"].to_numpy(dtype=float)
    pair_lower = pairwise["allocation_tv_ci_lower"].to_numpy(dtype=float)
    pair_upper = pairwise["allocation_tv_ci_upper"].to_numpy(dtype=float)
    ax_b.hlines(y_pair, pair_lower, pair_upper, linewidth=float(plots["line_width"]))
    ax_b.scatter(pair_point, y_pair, s=float(plots["marker_size"]) ** 2, zorder=3)
    ax_b.vlines(pair_lower, y_pair - cap, y_pair + cap, linewidth=0.8)
    ax_b.vlines(pair_upper, y_pair - cap, y_pair + cap, linewidth=0.8)
    ax_b.set_yticks(y_pair)
    ax_b.set_yticklabels(pairwise["pair_label"].tolist())
    pairwise_tv_upper = _dynamic_tv_upper(pair_upper)
    ax_b.set_xlim(0.0, pairwise_tv_upper)
    ax_b.set_xlabel("Pairwise source-route allocation TV")
    ax_b.grid(axis="x", alpha=0.25, linewidth=0.6)
    for y_value, row in zip(y_pair, pairwise.itertuples(index=False), strict=True):
        ax_b.text(
            1.02,
            y_value,
            _shared_annotation(
    ...
    base = output_dir / "figure_exp2_main"
    files = _save_figure(fig, base, config)
    plt.close(fig)

    metadata = {
        **run_metadata,
        "figure_id": "figure_exp2_main",
        "panel_definitions": {
            "a": "UID-bootstrap allocation TV from the arrival-time diagnostic anchor; annotations report shared top-k cells and their interval.",
            "b": "Horizontal dot-and-whisker comparisons of pairwise source-route allocation TV; annotations report shared top-k cells and their interval.",
        },
        "axis_definitions": {
            "a_x": "allocation_tv_vs_arrival",
            "b_x": "allocation_tv",
            "b_y": "fixed source-route pair order",
        },
        "panel_b_x_limit": [0.0, pairwise_tv_upper],
        "uncertainty_definition": "95% percentile UID-cluster bootstrap intervals.",
    ...
    metadata_path = output_dir / "figure_exp2_main_metadata.json"
...
def _cohort_display_table(
    ...
    rows = [
        ...
        ("Bootstrap unit", str(bootstrap_audit["bootstrap_unit"])),
        ("Bootstrap repetitions", f"{int(bootstrap_audit['bootstrap_repetitions']):,}"),
        (
...
def make_tables(
    ...
    *,
    ...
    cohort_csv = output_dir / "table_exp2_cohort.csv"
    cohort.to_csv(cohort_csv, index=False)
    paths.append(cohort_csv)
    cohort_tex = output_dir / "table_exp2_cohort.tex"
    ...
    for row in primary.itertuples(index=False):
        ...
        primary_display_rows.append(
            {
                "Route": row.route,
                "Allocation TV [95% CI]": _interval(
                    row.allocation_tv_vs_arrival,
                    row.allocation_tv_ci_lower,
                    row.allocation_tv_ci_upper,
                ),
                f"Top-{top_k} shared [95% CI]": _shared_interval(
                    row.top_k_overlap_vs_arrival,
                    row.top_k_overlap_ci_lower,
                    row.top_k_overlap_ci_upper,
                    top_k,
                ),
                "Kendall tau-b [95% CI]": _interval(
                    row.kendall_tau_b_vs_arrival,
                    row.kendall_tau_b_ci_lower,
                    row.kendall_tau_b_ci_upper,
                ),
                "Kendall support cells": int(row.common_active_support_count),
        ...
        pairwise_display_rows.append(
            {
                ...
                "Allocation TV [95% CI]": _interval(
                    row.allocation_tv, row.allocation_tv_ci_lower, row.allocation_tv_ci_upper
                ),
                f"Top-{top_k} shared [95% CI]": _shared_interval(
                    row.top_k_overlap,
                    row.top_k_overlap_ci_lower,
                ...
                "Common active cells": int(row.common_active_support_count),
                "Support frozen": "Yes" if bool(row.support_frozen) else "No",
            }
        )
