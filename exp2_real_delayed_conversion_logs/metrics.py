...
def stable_top_k(route_frame: pd.DataFrame, top_k: int) -> tuple[str, ...]:
    ...
    ordered = route_frame.sort_values(
        ["decision_cell_score", "campaign_id", "source_date_utc", "decision_cell_id"],
...
def kendall_tau_b(
    ...
    *,
    ...
    merged = left_frame[
        ["decision_cell_id", "decision_cell_score", "credited_conversion_mass"]
    ].merge(
        right_frame[
            ["decision_cell_id", "decision_cell_score", "credited_conversion_mass"]
        ],
    ...
    support = {
        "common_active_support_count": int(len(active)),
    ...
    result = kendalltau(
        active["decision_cell_score_left"].to_numpy(dtype=float),
        active["decision_cell_score_right"].to_numpy(dtype=float),
        variant="b",
        nan_policy="omit",
    )
    ...
    *,
    ...
    for route_id in PRIMARY_SOURCE_ROUTE_ORDER:
        metric_state = build_pairwise_metric_state(
            allocations, "arrival_bin_anchor", route_id
        )
        ...
        arrival_rows.append(
            {
                ...
                "ranking_displacement_at_k": row["ranking_displacement_at_k"],
                "kendall_tau_b_vs_arrival": row["kendall_tau_b"],
                "common_active_support_count": row["common_active_support_count"],
            ...
            pair.update(
                {
                    ...
                    "journey_count": int(len(journeys)),
                    "user_count": int(journeys["user_id"].nunique()),
                    "mean_journey_assignment_tv": compute_mean_journey_assignment_tv(
                        source_assignments,
                        left_route,
                        right_route,
    ...
    for top_k in top_k_values:
        for route_id in PRIMARY_SOURCE_ROUTE_ORDER:
            record = _pair_metrics_from_allocations(
                allocations, "arrival_bin_anchor", route_id, int(top_k)
            )
            record.update(
                {
