...
def validate_run(run_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    condition_level = pd.read_parquet(
        run_dir / "derived" / "module_b" / "exp4_module_b_condition_level.parquet"
    )
    c_replication = pd.read_parquet(
        run_dir / "derived" / "module_c" / "exp4_module_c_replication_level.parquet"
    )
    ...
    figure_complete = all(
        (
            ...
            / "figures"
            / extension
            / f"{figure_id}.{extension if extension != 'png' else 'png'}"
        ).exists()
    ...
    schema_checks = (
        ...
        (
            "module_b_condition_schema",
            *has_columns(condition_level, MODULE_B_CONDITION_COLUMNS),
        ),
    ...
    table_result: ValidationResult = validate_and_write_table_checks(
        run_dir, control_summary
    )
    contrasts = pd.read_csv(
        run_dir / "derived" / "module_a" / "exp4_module_a_paired_contrasts.csv"
    )
    precision_result = validate_monte_carlo_precision(
        contrasts, str(run_config["run_tier"])
    )
    ...
    engineering_rows = [
        _row(
            ...
            f"missing={[path.relative_to(run_dir).as_posix() for path in required if not path.exists()]}",
        ),
        _row(
            "result_schema_is_v3",
            run_config["result_schema"] == RESULT_SCHEMA
            and set(seed_level["result_schema"]) == {RESULT_SCHEMA},
        ...
        _row(
            "figure_bundles_complete",
            figure_complete,
            f"figure_count={len(figure_ids)}",
        ),
        ...
        _row(
            "main_table_exists",
            bool(
                table_result.checks.get("main_table_csv_exists", False)
                and table_result.checks.get("main_table_tex_exists", False)
            ),
            MAIN_TABLE_ID,
        ),
        _row(
            "main_table_has_required_rows",
            bool(table_result.checks.get("main_table_has_exactly_two_rows", False)),
            f"rows={table_result.details.get('main_table_row_count')}",
        ),
        _row(
            "main_table_values_finite",
            bool(table_result.checks.get("main_table_values_finite", False)),
            f"has_nan={table_result.details.get('main_table_has_nan')}",
        ),
        _row(
            "main_table_matches_source",
            bool(table_result.checks.get("main_table_matches_source", False)),
            f"mismatches={table_result.details.get('value_mismatches')}",
        ),
        _row(
            "main_table_latex_nonempty",
            bool(table_result.checks.get("main_table_latex_has_two_data_rows", False))
            and bool(
                table_result.checks.get("main_table_latex_has_data_beyond_rules", False)
            ),
            f"data_rows={table_result.details.get('main_table_latex_data_row_count')}",
        ),
        _row(
            "main_table_complete",
            table_result.passed,
            f"semantic_checks={sum(table_result.checks.values())}/{len(table_result.checks)}",
        ),
        _row(
            "primary_contrast_contract_valid",
            bool(precision_result.checks.get("primary_contrast_contract_valid", False)),
            f"count={precision_result.primary_contrast_count}",
        ),
        _row(
            "primary_monte_carlo_precision_pass",
            precision_result.engineering_pass(),
            precision_result.details,
        ),
        _row(
            "no_nonfull_precision_status_in_full_run",
            bool(
                precision_result.checks.get(
                    "no_nonfull_precision_status_in_full_run", False
                )
            ),
            f"run_tier={precision_result.run_tier}",
        ),
    ]
    engineering_rows.extend(
        _row(name, passed, details) for name, passed, details in schema_checks
    )
    scientific_rows = [
        _row(name, passed, details)
        for name, passed, details in scientific_checks(run_dir)
    ]
    scientific_rows.append(
        _row("FIGURE_SOURCE_RECONSTRUCTABLE", sources_ok, sources_details)
    )
    ...
    engineering = {
        ...
        "status": (
            "PASS"
            if all(row["status"] == "PASS" for row in engineering_rows)
            else "FAIL"
        ),
    ...
    scientific = {
        ...
        "status": (
            "PASS"
            if all(row["status"] == "PASS" for row in scientific_rows)
            else "FAIL"
        ),
