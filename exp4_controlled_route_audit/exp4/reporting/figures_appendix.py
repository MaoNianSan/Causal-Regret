...
def _heatmap(run_dir: Path, population: pd.DataFrame, source: Path) -> None:
    ...
    axis.set_xticks(
        range(matrix.shape[1]), labels=[f"{value:g}" for value in matrix.columns]
    )
    axis.set_yticks(
        range(matrix.shape[0]), labels=[f"{value:g}" for value in matrix.index]
    )
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[0],
        primary,
        source,
        "Module A population-defect grid.",
    )
...
def _contrast_forest(run_dir: Path, contrasts: pd.DataFrame, source: Path) -> None:
    ...
    axis.errorbar(
        data["paired_mean_difference"],
        y,
        xerr=1.96 * data["paired_mcse"],
        fmt="o",
        color="#2F6B9A",
        capsize=2,
    )
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[1],
        data,
        source,
        "Shared-seed route-label contrasts.",
    )


def _optimal_set_conflict(
    run_dir: Path, population: pd.DataFrame, source: Path
) -> None:
    ...
    for sigma in MODULE_A.proxy_noise_sds:
        group = data[np.isclose(data["attribution_proxy_noise_sd"], sigma)].sort_values(
            "route_label_rate"
        )
        axis.plot(
            group["route_label_rate"],
            group["route_optimal_set_conflict_rate_mean"],
            marker="o",
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[2],
        data,
        source,
        "Optimal-set conflict, distinct from pairwise sign disagreement.",
    )
...
def _ambiguity_relation(run_dir: Path, selection: pd.DataFrame, source: Path) -> None:
    ...
    axis.plot(
        data["mean_ambiguity"],
        data["mean_true_unit_pairwise_discrepancy"],
        marker="o",
        color="#C75B39",
    )
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[3],
        data,
        source,
        "Ambiguity-decile relation to true unit defect.",
    )
...
def _weight_figures(run_dir: Path, weights: pd.DataFrame, source: Path) -> None:
    ...
    figure, axis = plt.subplots(figsize=(4.8, 3.0))
    axis.plot(
        data["audit_evidence_rate"],
        data["mean_weight_p95"],
        marker="o",
        label="p95 weight",
    )
    axis.plot(
        data["audit_evidence_rate"],
        data["mean_weight_max"],
        marker="s",
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[4],
        data,
        source,
        "IPW weight diagnostics and positivity support.",
    )
    figure, axis = plt.subplots(figsize=(5.2, 3.0))
    axis.plot(
        data["audit_evidence_rate"],
        data["mean_labelled_sample_size"],
        marker="o",
        label=r"$n_{lab}$",
    )
    axis.plot(
        data["audit_evidence_rate"],
        data["mean_effective_sample_size"],
        marker="s",
        label=r"$n_{eff}$",
    )
    axis.set_xlabel(r"$\rho_{audit}$")
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[5],
        data,
        source,
        "Labelled and effective support for selective IPW.",
    )
...
def _parameter_recovery(run_dir: Path, parameters: pd.DataFrame, source: Path) -> None:
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[6],
        data,
        source,
        "Affine-control intercept and slope recovery.",
    )
...
def _correspondence(run_dir: Path, checks: pd.DataFrame, source: Path) -> None:
    ...
    figure, axis = plt.subplots(figsize=(4.8, 3.0))
    values = [
        float(data["pre_mean_abs_pearson"].iloc[0]),
        float(data["post_mean_abs_pearson"].iloc[0]),
    ]
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[7],
        data,
        source,
        "Correspondence before and after blocked permutation.",
    )
...
def _attribution(run_dir: Path, seed_level: pd.DataFrame, source: Path) -> None:
    rows = seed_level[
        ...
        & np.isclose(
            seed_level["attribution_proxy_noise_sd"], MODULE_A.primary_proxy_noise_sd
        )
    ]
    data = (
        rows.groupby("route_label_rate", sort=True)
        .agg(
    ...
    for column, label, marker in (
        ("mean_entropy", "Entropy", "o"),
        ("mean_max_mass", "Maximum mass", "s"),
        ("mean_true_source_mass", "True-source mass", "^"),
    ):
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[8],
        data,
        source,
        "Attribution diagnostics remain appendix-only.",
    )


def _robustness(run_dir: Path, population: pd.DataFrame, source: Path) -> None:
    smooth = population[
        population["route_id"] == "proxy_label_smooth_robustness"
    ].sort_values("route_label_rate")
    primary = population[
        (population["route_id"] == "proxy_label")
        & np.isclose(
            population["attribution_proxy_noise_sd"], MODULE_A.primary_proxy_noise_sd
        )
    ].sort_values("route_label_rate")
    data = pd.concat(
        (
            primary.assign(dgp="Primary parity-floor DGP"),
            smooth.assign(dgp="Smooth no-parity-floor DGP"),
        ),
        ignore_index=True,
    )
    figure, axis = plt.subplots(figsize=(5.0, 3.0))
    for label, group in data.groupby("dgp", sort=False):
        axis.plot(
            group["route_label_rate"],
            group["population_action_gap_defect_mean"],
            marker="o",
            label=label,
        )
    axis.set_xlabel(r"$q_{route}$")
    ...
    _save(
        figure,
        run_dir,
        APPENDIX_FIGURE_IDS[9],
        data,
        source,
        "Smooth-loss robustness without parity-dependent floors.",
    )
