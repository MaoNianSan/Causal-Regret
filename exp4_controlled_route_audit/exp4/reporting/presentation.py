...
MAIN_CONTRACT = {
    ...
    "panel_a_marker_registry": {
        str(sigma): marker for sigma, marker in SIGMA_PROXY_MARKERS.items()
    },
    "panel_a_exclusions": [
        ...
        rows.append(
            {
                ...
                "interval_lower": (
                    pd.NA
                    if deterministic
                    else row.mean_pairwise_gap_discrepancy_ci_lower
                ),
                "interval_upper": (
                    pd.NA
                    if deterministic
                    else row.mean_pairwise_gap_discrepancy_ci_upper
                ),
                "uncertainty_role": (
                    "deterministic controlled endpoint"
                    if deterministic
                    else "paired-seed interval from frozen summary"
                ),
                "uncertainty_method": (
                    "none" if deterministic else "frozen paired-seed summary"
                ),
            ...
            rows.append(
                {
                    ...
                    "interval_lower": (
                        pd.NA if deterministic else estimate - 1.96 * mcse
                    ),
                    "interval_upper": (
                        pd.NA if deterministic else estimate + 1.96 * mcse
                    ),
                    "uncertainty_role": (
                        "deterministic full-population endpoint"
                        if deterministic
                        else "estimate +/- 1.96 Monte Carlo standard error"
                    ),
    ...
    *,
    ...
    for panel_index, path in enumerate(paths):
        ...
        if figure_id == "exp4_appendix_route_alignment_detail" and panel_index < 2:
            ...
            axis.set_yticks(
                np.arange(len(matrix.index)), [f"{value:g}" for value in matrix.index]
            )
            ...
            plotted = [
                (index, metric, float(row[metric])) for index, row in frame.iterrows()
            ]
        elif figure_id == "exp4_appendix_route_alignment_detail":
            metric = "mean_pairwise_gap_discrepancy_mean"
            for (dgp, sigma), group in frame.groupby(
                ["dgp", "attribution_proxy_noise_sd"], sort=True
            ):
                group = group.sort_values("route_label_rate")
                axis.plot(
                    group.route_label_rate,
                    group[metric],
                    marker="o",
                    linewidth=0.8,
                    label=f"{dgp}, sigma={sigma:g}",
                )
            axis.set_xlabel(r"$q_{route}$")
            ...
            metric = (
                ...
                if panel_index == 0
                else "mean_weight_p95" if panel_index == 1 else "rmse"
            )
            ...
            for position, (index, row) in enumerate(frame.iterrows()):
                axis.plot(
                    [row.pre_mean_abs_spearman, row.post_mean_abs_spearman],
                    [position, position],
                    color="0.45",
                )
                axis.plot(
                    row.pre_mean_abs_spearman, position, marker="o", color="#b54708"
                )
                axis.plot(
                    row.post_mean_abs_spearman,
                    position,
                    marker="s",
                ...
                plotted.extend(
                    [
                        (
                            index,
                            "pre_mean_abs_spearman",
                            float(row.pre_mean_abs_spearman),
                        ),
                        (
                            index,
                            "post_mean_abs_spearman",
                            float(row.post_mean_abs_spearman),
                        ),
                    ]
                )
            axis.set_yticks(
            ...
            axis.scatter(
                frame.true_value, frame.mean_estimate, color="#1f4e79", marker="o"
            )
            bounds = [
                min(frame.true_value.min(), frame.mean_estimate.min()),
                max(frame.true_value.max(), frame.mean_estimate.max()),
            ]
            axis.plot(bounds, bounds, color="0.55", linestyle="--", linewidth=0.7)
            axis.set_xlabel("True parameter")
            axis.set_ylabel("Mean estimate")
            plotted = [
                (index, "mean_estimate", float(row.mean_estimate))
            ...
            for position, (index, row) in enumerate(frame.iterrows()):
                ...
                axis.plot(
                    row.raw_pairwise_discrepancy, position, marker="o", color="#b54708"
                )
                axis.plot(
                    row.oof_calibrated_pairwise_discrepancy,
                    position,
                ...
                plotted.extend(
                    [
                        (
                            ...
                            float(row.raw_pairwise_discrepancy),
                        ),
                        (
                            index,
                            "oof_calibrated_pairwise_discrepancy",
                            float(row.oof_calibrated_pairwise_discrepancy),
                        ),
                    ]
                )
            axis.set_yticks(
            ...
            long_rows.append(
                {
                    ...
                    "estimand_id": (
                        "D_pair"
                        if metric == "mean_pairwise_gap_discrepancy_mean"
                        else metric
                    ),
...
def render_presentation(
    ...
    for sigma, group in boundary.groupby("attribution_proxy_noise_sd", sort=True):
        ...
        color = plt.cm.viridis(
            float(sigma) / max(float(boundary.attribution_proxy_noise_sd.max()), 1.0)
        )
        ...
        axes[0, 0].errorbar(
            ...
            yerr=[
                uncertain.mean_pairwise_gap_discrepancy_mean
                - uncertain.mean_pairwise_gap_discrepancy_ci_lower,
                uncertain.mean_pairwise_gap_discrepancy_ci_upper
                - uncertain.mean_pairwise_gap_discrepancy_mean,
            ],
            color=color,
            marker=marker,
    ...
    selected_controls = (
        controls.set_index("control_id").reindex(CALIBRATION_CONTROLS).reset_index()
    )
    ...
    weights = (
        source.source_run / "derived/module_b/exp4_module_b_weight_diagnostics.csv"
    )
    appendix_groups = [
        (
    ...
    for filename, stem, semantics in (
        (
            "tbl_app_exp4_parameters.csv",
            "tbl_app_exp4_parameters",
            "Frozen v3 parameter table.",
        ),
        (
            "tbl_app_exp4_paired_contrasts.csv",
            "tbl_app_exp4_paired_contrasts",
            "Complete Module-A D_pair and secondary diagnostic table.",
        ),
        (
            "tbl_app_exp4_audit_performance.csv",
            "tbl_app_exp4_audit_performance",
            "Complete audit-performance table.",
        ),
        (
            "tbl_exp4_calibration_controls.csv",
            "tbl_exp4_calibration_controls",
            "Calibration-control table.",
        ),
    ):
