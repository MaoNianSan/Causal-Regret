    ...
    **kwargs: Any,
    ...
    if valid_ci:
        ...
        ax.errorbar(
            ...
            capsize=capsize,
            elinewidth=max(plot_linewidth, 1.05),
            capthick=max(plot_linewidth, 1.05),
            zorder=zorder,
            **kwargs,
        )
    else:
...
def _save(
    ...
    for ext in ["pdf", "png"]:
        fig.savefig(
            run_dir / "figures" / ext / f"{stem}.{ext}",
            dpi=config.PAPER_DPI,
            bbox_inches="tight",
        )
    ...
    if not data_path.exists():
        data[
            BASE_FIGURE_COLUMNS
            + [c for c in data.columns if c not in BASE_FIGURE_COLUMNS]
    ...
    metadata["protected_csv_sha256_unchanged"] = (
        protected_hashes_before == protected_hashes_after
    )
    (run_dir / "figures" / "metadata" / f"{stem}_metadata.json").write_text(
    ...
    *,
    ...
    row = {
        ...
        **_method_fields(method_id),
        ...
        "n_users": np.nan,
        "filter_id": (
            "primary_setting_beta_200"
            if panel_id in {"a", "b", "c"}
            else "appendix_beta_sweep"
        ),
        "filter_description": (
            "controlled structural_high setting with beta=2.00"
            if panel_id in {"a", "b", "c"}
            else "matched mean-delay coupling diagnostic"
        ),
        "run_mode": run_config["mode"],
        "paper_result": bool(run_config.get("paper_result", False)),
        "notes": notes,
    }
    if extra:
        row.update(extra)
...
def run(run_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(
        1, 3, figsize=(config.PAPER_FIGURE_WIDTH_IN, 2.65), constrained_layout=True
    )
    ...
    if (
        np.isfinite(x_low).all()
        and np.isfinite(x_high).all()
        and np.isfinite(y_low).all()
        and np.isfinite(y_high).all()
    ):
        axes[0].errorbar(
            x,
            y,
    ...
    axes[0].set_ylabel(
        "Loss-map distortion (diagnostic)", fontsize=config.AXIS_LABEL_FONT_SIZE
    )
    axes[0].text(
        0.0,
        1.03,
        ...
        fontsize=config.PANEL_LABEL_FONT_SIZE,
        fontweight="bold",
    )
    for _, row in p.iterrows():
        figure_rows.append(
            _figure_row(
                figure_id="fig_exp4_recoverability_boundary",
                panel_id="a",
                run_config=run_config,
                subexperiment_id="proxy_distortion_diagnostic",
                setting_id=f"sigma_{row.proxy_noise_sigma:.2f}",
                method_id="proxy_noisy_oracle_diagnostic",
                ...
                x_value=row.proxy_state_error_per_round_mean,
                x_display_label="Proxy-state error",
                y_value=row.absolute_proxy_distortion_per_round_mean,
                ci_lower=row.absolute_proxy_distortion_per_round_ci_low,
                ci_upper=row.absolute_proxy_distortion_per_round_ci_high,
                n_seeds=row.n_seeds,
                n_bootstrap=row.n_bootstrap,
                uncertainty_unit=row.uncertainty_unit,
                notes="Diagnostic control; point label gives proxy noise sigma.",
                extra={
                    "x_ci_lower": row.proxy_state_error_per_round_ci_low,
                    "x_ci_upper": row.proxy_state_error_per_round_ci_high,
                    "proxy_noise_sigma": row.proxy_noise_sigma,
                },
            )
        )
    ...
    source = source[
        np.isclose(source["proxy_noise_sigma"], config.DEFAULT_PROXY_SIGMA)
    ].copy()
    recovery = source[source["method_id"].eq("proxy_label_recovery")].sort_values(
        "source_label_rate_q"
    )
    ...
    axes[1].axhline(
        arrival.causal_regret_per_round_mean,
        linestyle="--",
        linewidth=config.LINE_WIDTH,
    )
    axes[1].axhline(
        history.causal_regret_per_round_mean, linestyle=":", linewidth=config.LINE_WIDTH
    )
    _errorbar(
        axes[1],
        recovery["source_label_rate_q"].to_numpy(float),
        recovery["causal_regret_per_round_mean"].to_numpy(float),
        recovery["causal_regret_per_round_ci_low"].to_numpy(float),
        recovery["causal_regret_per_round_ci_high"].to_numpy(float),
        marker="o",
        linestyle="-",
        markersize=2.7,
        capsize=4.0,
    ...
    if np.isfinite(reference.causal_regret_per_round_ci_low) and np.isfinite(
        reference.causal_regret_per_round_ci_high
    ):
        axes[1].errorbar(
            [1.0],
            [reference.causal_regret_per_round_mean],
            yerr=[
                [
                    reference.causal_regret_per_round_mean
                    - reference.causal_regret_per_round_ci_low
                ],
                [
                    reference.causal_regret_per_round_ci_high
                    - reference.causal_regret_per_round_mean
            ...
            linestyle="None",
            capsize=4.0,
            markersize=config.MARKER_SIZE + 0.8,
    ...
    axes[1].annotate(
        "Arrival time",
        (q_max, arrival.causal_regret_per_round_mean),
        xytext=(-62, -10),
        textcoords="offset points",
        fontsize=6.3,
    )
    axes[1].annotate(
        "History surrogate",
        (q_max, history.causal_regret_per_round_mean),
        xytext=(-74, -11),
        textcoords="offset points",
        fontsize=6.3,
    )
    ...
    axes[1].annotate(
        "Proxy-label recovery",
        (anchor.source_label_rate_q, anchor.causal_regret_per_round_mean),
        xytext=(3, 7),
        textcoords="offset points",
        fontsize=6.3,
    )
    ...
    for row, label, note in [
        ...
        (
            ...
            "Horizontal reference; invariant in q.",
        ),
        (
            reference,
            "source_labelled_reference",
            "Reference point at q=1; same action trace as proxy-label recovery at q=1.",
        ),
    ]:
        figure_rows.append(
            _figure_row(
                ...
                panel_id="b",
                run_config=run_config,
                subexperiment_id="source_label_sweep",
                setting_id=f"q_{float(row.source_label_rate_q):.2f}_sigma_{config.DEFAULT_PROXY_SIGMA:.2f}",
                method_id=label,
                metric_id="causal_regret_per_round",
                metric_formula_id="post_warmup_structural_regret",
                x_id="source_label_rate_q",
                x_value=float(row.source_label_rate_q),
                x_display_label="Source-ID retention q",
                y_value=row.causal_regret_per_round_mean,
                ci_lower=row.causal_regret_per_round_ci_low,
                ci_upper=row.causal_regret_per_round_ci_high,
                n_seeds=row.n_seeds,
                n_bootstrap=row.n_bootstrap,
                uncertainty_unit=row.uncertainty_unit,
                notes=note,
    ...
    for _, row in recovery.iterrows():
        figure_rows.append(
            _figure_row(
                ...
                run_config=run_config,
                subexperiment_id="source_label_sweep",
                setting_id=f"q_{row.source_label_rate_q:.2f}_sigma_{config.DEFAULT_PROXY_SIGMA:.2f}",
                method_id="proxy_label_recovery",
                metric_id="causal_regret_per_round",
                metric_formula_id="post_warmup_structural_regret",
                x_id="source_label_rate_q",
                x_value=row.source_label_rate_q,
                x_display_label="Source-ID retention q",
                y_value=row.causal_regret_per_round_mean,
                ci_lower=row.causal_regret_per_round_ci_low,
                ci_upper=row.causal_regret_per_round_ci_high,
    ...
    matrix = phase.pivot(
        index="proxy_noise_sigma",
        columns="source_label_rate_q",
        values="oracle_normalized_recovery_mean",
    ).reindex(index=sigma_values, columns=q_values)
    image = axes[2].imshow(
        matrix.to_numpy(float), origin="lower", aspect="auto", vmin=0.0, vmax=1.0
    )
    axes[2].set_xticks(np.arange(len(q_values)), [f"{q:g}" for q in q_values])
    axes[2].set_yticks(
        np.arange(len(sigma_values)), [f"{sigma:g}" for sigma in sigma_values]
    )
    axes[2].set_xlabel("Source-ID retention q", fontsize=config.AXIS_LABEL_FONT_SIZE)
    axes[2].set_ylabel(
        "Attribution-proxy noise σ", fontsize=config.AXIS_LABEL_FONT_SIZE
    )
    axes[2].text(
        0.0,
        1.03,
    ...
    cbar = fig.colorbar(
        image, ax=axes[2], fraction=0.046, pad=0.04, ticks=[0.0, 0.5, 1.0]
    )
    ...
    for _, row in phase.iterrows():
        figure_rows.append(
            _figure_row(
                ...
                panel_id="c",
                run_config=run_config,
                subexperiment_id="recoverability_phase_map",
                setting_id=f"q_{row.source_label_rate_q:.2f}_sigma_{row.proxy_noise_sigma:.2f}",
                method_id="proxy_label_recovery",
                metric_id="oracle_normalized_recovery",
                metric_formula_id="arrival_oracle_normalized_recovery",
                x_id="source_label_rate_q",
                x_value=row.source_label_rate_q,
                x_display_label="Source-label retention q",
                y_value=row.proxy_noise_sigma,
                ci_lower=row.oracle_normalized_recovery_ci_low,
                ci_upper=row.oracle_normalized_recovery_ci_high,
                n_seeds=row.n_seeds,
                n_bootstrap=row.n_bootstrap,
                uncertainty_unit=row.uncertainty_unit,
                ...
                extra={
                    "proxy_noise_sigma": row.proxy_noise_sigma,
                    "heatmap_value": row.oracle_normalized_recovery_mean,
    ...
    _save(
        fig,
        run_dir,
        "fig_exp4_recoverability_boundary",
        pd.DataFrame(figure_rows),
        {
            "figure_id": "fig_exp4_recoverability_boundary",
            "figure_size_in": [config.PAPER_FIGURE_WIDTH_IN, 2.65],
            "dpi": config.PAPER_DPI,
            "mode": mode,
            "paper_result": bool(run_config.get("paper_result", False)),
            "figure_status": "fast_preview" if mode == "fast" else "paper_result",
            "panels": [
            ...
            "ci_level": config.CI_LEVEL if mode == "full" else None,
            "n_bootstrap": config.BOOTSTRAP_N if mode == "full" else 0,
            "input_data_status": "complete_simulator",
            "uses_latent_oracle_as_plotted_method": False,
            "uses_latent_oracle_normalization": True,
            "phase_map_metric_raw_not_clipped": True,
            "phase_map_display_vmin": 0.0,
            "phase_map_display_vmax": 1.0,
    ...
    fig2, axes2 = plt.subplots(
        1, 2, figsize=(config.PAPER_FIGURE_WIDTH_IN, 2.55), constrained_layout=True
    )
    mechanism = coupling[coupling["method_id"].eq("arrival_time_naive")].sort_values(
        "delay_state_coupling_beta"
    )
    _errorbar(
        axes2[0],
        mechanism["delay_state_coupling_beta"].to_numpy(float),
        mechanism["ranking_reversal_rate_mean"].to_numpy(float),
        mechanism["ranking_reversal_rate_ci_low"].to_numpy(float),
        mechanism["ranking_reversal_rate_ci_high"].to_numpy(float),
        marker="o",
        linestyle="-",
    )
    axes2[0].set_xlabel("Delay-state coupling β", fontsize=config.AXIS_LABEL_FONT_SIZE)
    axes2[0].set_ylabel(
        "Source-arrival ranking reversal", fontsize=config.AXIS_LABEL_FONT_SIZE
    )
    axes2[0].text(
        0.0,
        1.03,
    ...
    for _, row in mechanism.iterrows():
        appendix_rows.append(
            _figure_row(
                figure_id="fig_app_exp4_delay_state_coupling",
                panel_id="a",
                run_config=run_config,
                subexperiment_id="delay_state_coupling_diagnostic",
                setting_id=f"beta_{row.delay_state_coupling_beta:.2f}",
                method_id="arrival_time_naive",
                metric_id="ranking_reversal_rate",
                metric_formula_id="source_arrival_ranking_reversal",
                x_id="delay_state_coupling_beta",
                x_value=row.delay_state_coupling_beta,
                x_display_label="Delay-state coupling beta",
                y_value=row.ranking_reversal_rate_mean,
                ci_lower=row.ranking_reversal_rate_ci_low,
                ci_upper=row.ranking_reversal_rate_ci_high,
                n_seeds=row.n_seeds,
                n_bootstrap=row.n_bootstrap,
                uncertainty_unit=row.uncertainty_unit,
            )
    ...
    _errorbar(
        axes2[1],
        adv["delay_state_coupling_beta"].to_numpy(float),
        adv["source_binding_advantage_mean"].to_numpy(float),
        adv["source_binding_advantage_ci_low"].to_numpy(float),
        adv["source_binding_advantage_ci_high"].to_numpy(float),
        marker="o",
        linestyle="-",
    )
    ...
    axes2[1].set_ylabel(
        "Arrival minus source-bound regret", fontsize=config.AXIS_LABEL_FONT_SIZE
    )
    axes2[1].text(
        0.0,
        1.03,
    ...
    for _, row in adv.iterrows():
        appendix_rows.append(
            _figure_row(
                ...
                subexperiment_id="delay_state_coupling_diagnostic",
                setting_id=f"beta_{row.delay_state_coupling_beta:.2f}",
                method_id="source_labelled_reference",
                metric_id="source_binding_advantage",
                metric_formula_id="arrival_time_regret_minus_source_labelled_reference_regret",
                x_id="delay_state_coupling_beta",
                x_value=row.delay_state_coupling_beta,
                x_display_label="Delay-state coupling beta",
                y_value=row.source_binding_advantage_mean,
                ci_lower=row.source_binding_advantage_ci_low,
                ci_upper=row.source_binding_advantage_ci_high,
                n_seeds=row.n_seeds,
                n_bootstrap=row.n_bootstrap,
                uncertainty_unit=row.uncertainty_unit,
    ...
    _save(
        fig2,
        run_dir,
        "fig_app_exp4_delay_state_coupling",
        pd.DataFrame(appendix_rows),
        {
            "figure_id": "fig_app_exp4_delay_state_coupling",
            "figure_size_in": [config.PAPER_FIGURE_WIDTH_IN, 2.55],
            "dpi": config.PAPER_DPI,
            "mode": mode,
            "paper_result": bool(run_config.get("paper_result", False)),
            "figure_status": "appendix_diagnostic",
            "appendix_only": True,
            "panels": ["coupling mechanism diagnostic", "source-binding advantage"],
            "input_data_status": "complete_simulator",
            "ci_level": config.CI_LEVEL if mode == "full" else None,
            "n_bootstrap": config.BOOTSTRAP_N if mode == "full" else 0,
