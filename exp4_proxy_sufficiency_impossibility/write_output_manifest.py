...
def run(run_dir: Path) -> None:
    ...
    (run_dir/'logs'/'output_manifest.json').write_text(json.dumps({'run_dir':str(run_dir),'files':files},indent=2),encoding='utf-8')
    import pandas as pd
    pd.DataFrame(files).to_csv(run_dir/'logs'/'output_manifest.csv',index=False)

def main()->None:
    p=argparse.ArgumentParser(); p.add_argument('--run-dir',type=Path,required=True); args=p.parse_args(); run(args.run_dir)
