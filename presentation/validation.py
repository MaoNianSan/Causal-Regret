...
def _validate_bundle(
    ...
    figure_id: str,
    section: str,
    expected_paper_result: bool = False,
) -> list[dict[str, Any]]:
    ...
    for label, path in paths.items():
        results.append(
            {
                "check": f"{figure_id}:{label}:nonempty",
    ...
    results.append(
        {
            "check": f"{figure_id}:long_form_schema",
            "passed": not missing,
            "details": ", ".join(missing) if missing else "complete",
        }
    )
    ...
    results.extend(
        [
            {
                "check": f"{figure_id}:spec",
                "passed": metadata.get("spec_id") == SPEC_ID,
                "details": str(metadata.get("spec_id")),
            },
            {
                "check": f"{figure_id}:paper_result_contract",
                "passed": metadata.get("paper_result") is expected_paper_result,
                "details": str(metadata.get("paper_result")),
            },
            {
                "check": f"{figure_id}:source_hashes",
                "passed": bool(metadata.get("source_file_hashes")),
                "details": str(len(metadata.get("source_file_hashes", {}))),
            },
            {
                "check": f"{figure_id}:three_graphic_hashes",
                "passed": set(metadata.get("figure_file_hashes", {}))
                == {paths["pdf"].name, paths["svg"].name, paths["png"].name},
                "details": str(metadata.get("figure_file_hashes", {}).keys()),
            },
        ]
    )
    ...
    results.append(
        {
            "check": f"{figure_id}:svg_live_text",
            "passed": "<text" in svg_text,
            "details": "SVG text elements",
        }
    )
    ...
    for filename, digest in metadata.get("figure_file_hashes", {}).items():
        file_path = (
            paths["pdf"].parent / filename
            if filename.endswith(".pdf")
            else (
                paths["svg"].parent / filename
                if filename.endswith(".svg")
                else paths["png"].parent / filename
            )
        )
        results.append(
            {
                "check": f"{figure_id}:hash:{filename}",
                "passed": file_path.exists() and sha256_file(file_path) == digest,
    ...
    results = [
        {
            ...
            "passed": not source.missing_files(),
            "details": ", ".join(map(str, source.missing_files())),
        }
    ]
    if source.experiment == "Exp1":
        payload = json.loads(
            (source.source_run / "metadata/exp1_run_lineage.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        results.append(
            {
                "check": "exp1_scientific_generation_config_hash",
                "passed": observed == source.config_hash,
                "details": str(observed),
            }
        )
    elif source.experiment == "Exp4":
        payload = json.loads(
            (source.source_run / "logs/exp4_stage_config_migration.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        results.append(
            {
                "check": "exp4_scientific_config_hash",
                "passed": observed == source.config_hash,
                "details": str(observed),
            }
        )
        run = json.loads(
            (source.source_run / "logs/run_config.json").read_text(encoding="utf-8")
        )
        results.extend(
            [
                {
                    "check": "exp4_v3_schema",
                    "passed": run.get("result_schema")
                    == "exp4_controlled_route_audit_v3",
                    "details": str(run.get("result_schema")),
                },
                {
                    "check": "exp4_source_paper_result_matches",
                    "passed": run.get("paper_result")
                    == source.scientific_source_paper_result,
                    "details": str(run.get("paper_result")),
                },
            ]
        )
        ...
        row = frame.loc[
            frame.route_id.eq("arrival_carrier"),
            "maximum_heldout_reference_pair_gap_error",
        ]
        actual = float(row.iloc[0]) if len(row) == 1 else float("nan")
        results.append(
            {
                "check": "exp3_current_arrival_max_gap_source",
                "passed": abs(actual - 0.6417907611) < 1e-9,
                "details": repr(actual),
            }
        )
    return results
...
def validate_preview(
    ...
    for figure_id in figure_ids:
        results.extend(
            _validate_bundle(
                layout, figure_id, "main", expected_paper_result=source.paper_result
            )
        )
        ...
        for figure_id in appendix.get("figure_ids", []):
            results.extend(
                _validate_bundle(
                    ...
                    figure_id,
                    "appendix",
                    expected_paper_result=source.paper_result,
                )
            )
    results.append(
        {
            "check": "presentation_manifest_exists",
