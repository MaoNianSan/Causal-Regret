...
def _record(rows: list[dict[str, str]], name: str, ok: bool, detail: str = "") -> bool:
    rows.append({"check_name": _safe_text(name), "status": "PASSED" if ok else "FAILED", "details": _safe_text(detail)})
    return bool(ok)
...
def check_project(mode: str, output_tag: str | None = None) -> bool:
    ...
    if manifest:
        ...
        _record(rows, "run mode matches requested check", str(manifest.get("mode")) == str(mode), str(manifest.get("mode")))
        ...
        _record(rows, "bootstrap contract", int(manifest.get("n_bootstrap", -1)) == N_BOOTSTRAP and float(manifest.get("ci_level", -1.0)) == CI_LEVEL, f"n_bootstrap={manifest.get('n_bootstrap')}; ci_level={manifest.get('ci_level')}")
        _record(rows, "input status declared", manifest.get("input_data_status") == INPUT_DATA_STATUS, str(manifest.get("input_data_status")))
        detailed = bool(manifest.get("detailed_trace_logs", False))
        trace_files = [root / "raw" / name for name in ("delay_schedule.csv", "arrival_log.csv", "step_log.csv")]
        if detailed:
            _record(rows, "detailed trace files are populated", all(_nonempty_csv(path) for path in trace_files), "; ".join(str(path) for path in trace_files))
        else:
            _record(rows, "summary-only run has no placeholder trace files", not any(path.exists() for path in trace_files), "; ".join(str(path) for path in trace_files if path.exists()))
        if mode == "fast":
            _record(rows, "fast result is not paper-marked", not bool(manifest.get("paper_result")), f"paper_result={manifest.get('paper_result')}")
    ...
    if seed_path.exists():
        try:
            ...
            required = {
                "seed", "delay_setting", "setting_id", "regime", "method", "method_id",
                "method_display_name", "information_interface", "reference_role", "deployable",
                "final_Rc", "mean_delay", "context_observed_by_all", "regret_comparator",
                "delay_path_id", "state_path_id", "effective_feedback_units", "n_observed_arrivals",
                "em_delay_likelihood", "labelled_feature_alignment_max", "run_mode", "paper_result",
                "input_data_status", "loss_map_mismatch_rate", "delta_attr_event_per_arrival",
            ...
            _record(rows, "regret comparator is context-information oracle", set(seed["regret_comparator"].astype(str)) == {"context_information_oracle"}, str(seed["regret_comparator"].drop_duplicates().tolist()))
            _record(rows, "causal regret finite", bool(np.isfinite(pd.to_numeric(seed["final_Rc"], errors="coerce")).all()), "final_Rc")
            _record(rows, "canonical run metadata", set(seed["run_mode"].astype(str)) == {mode} and set(seed["input_data_status"].astype(str)) == {INPUT_DATA_STATUS}, f"run_mode={seed['run_mode'].drop_duplicates().tolist()}")
            _record(rows, "primary metric is declared", set(seed["metric_id"].astype(str)) == {PRIMARY_METRIC}, str(seed["metric_id"].drop_duplicates().tolist()))

            primary = seed[seed["delay_setting"].isin(PRIMARY_MATCHED_SETTINGS)]
            path_count = primary.groupby(["seed", "delay_setting"])["delay_path_id"].nunique().max() if not primary.empty else np.inf
            _record(rows, "primary paths shared across methods", int(path_count) == 1, f"max distinct path IDs={path_count}")
            mean_spread = primary.groupby(["seed", "delay_setting"])["mean_delay"].agg(lambda values: float(np.nanmax(values) - np.nanmin(values))).max() if not primary.empty else np.inf
            ...
            structural = {"state_structural_matched_15", "proxy_good_matched_15", "proxy_bad_matched_15", "action_structural_stress"}
            em = seed[seed["delay_setting"].isin(structural) & seed["method"].eq("causal_em")]
            _record(rows, "structural EM uses observable-state integrated likelihood", set(em["em_delay_likelihood"].astype(str)) == {"gaussian_observable_state_integrated_quadrature"}, str(em["em_delay_likelihood"].drop_duplicates().tolist()))
            ablation = seed[seed["delay_setting"].isin(structural) & seed["method"].eq("causal_em_misspecified")]
            _record(rows, "EM ablation is explicitly stationary", set(ablation["em_delay_likelihood"].astype(str)) == {"stationary_geometric_ablation"}, str(ablation["em_delay_likelihood"].drop_duplicates().tolist()))
            ...
            _record(rows, "EM/proxy labelled updates share decision feature space", bool((alignment.fillna(np.inf) <= 1e-12).all()), f"max discrepancy={float(alignment.max()) if len(alignment) else np.nan}")
    ...
    if summary_path.exists():
        ...
        required_summary = {"mean_causal_regret_per_round", "ci_lower", "ci_upper", "ci_level", "n_bootstrap", "ci_method"}
        _record(rows, "method summary bootstrap schema", required_summary.issubset(summary.columns), f"missing={sorted(required_summary-set(summary.columns))}")
        _record(rows, "method summary uses 2000 percentile bootstrap", bool((pd.to_numeric(summary["n_bootstrap"], errors="coerce") == N_BOOTSTRAP).all()) and set(summary["ci_method"].astype(str)) == {"percentile_bootstrap"}, f"n_bootstrap={summary['n_bootstrap'].drop_duplicates().tolist()}")
    else:
        _record(rows, "method summary exists", False, str(summary_path))

    trajectory = root / "processed" / "selected_trajectory_points.csv"
    _record(rows, "selected trajectory output exists", _nonempty_csv(trajectory), str(trajectory))
    ...
    _record(rows, "figure data follow common schema", not schema_failures, "; ".join(schema_failures))

    ok = all(row["status"] == "PASSED" for row in rows)
    report = pd.DataFrame(rows)
    report.to_csv(root / "checks" / "self_check_report.csv", index=False)
    with (root / "checks" / "self_check_report.md").open("w", encoding="utf-8") as handle:
