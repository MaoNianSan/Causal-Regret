...
def main() -> int:
    ...
    if manifest.get("backend_status") != "completed" or manifest.get("mode") != "full" or bool(manifest.get("is_smoke")):
    ...
    for directory in (root / "raw", root / "summaries", root / "tables", root / "processed"):
        if directory.exists():
    ...
    manifest["paper_result_gate"] = "passed: completed full run plus self-check plus explicit finalization"
